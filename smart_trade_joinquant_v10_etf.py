# -*- coding: utf-8 -*-
"""
智能ETF量化交易策略 V10.0 - 自适应动量择时
=============================================
设计原则：
  1. 抗过拟合：使用学术界验证有效的方法，参数取公认默认值
  2. 无未来函数：所有数据取前一交易日，避免 FutureDataError
  3. 自适应：ATR动态止损替代固定百分比，波动大时止损宽、波动小时止损紧
  4. 动量优选：在多只ETF中选趋势最强的，而非随机选
  5. 简化信号：4买+4卖，每个条件有明确理论依据

核心改进 vs V8.0：
  - ATR动态止损（2.5倍ATR）替代固定5%止损 → 趋势行情中不被轻易震出
  - 风险调整后动量排序 → 选最强的ETF买入
  - 波动率反比仓位 → 高波减仓、低波加仓
  - 趋势跟踪模式 → 强趋势中只用止损退出，不用信号卖出
  - 更快的趋势识别 → 用EMA替代部分MA，加入动量因子
