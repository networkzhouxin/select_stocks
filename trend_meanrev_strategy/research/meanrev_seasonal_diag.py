# -*- coding: utf-8 -*-
"""季节性品种日线诊断：512600 主要消费 / 512690 酒ETF。

复用周期品种诊断口径。消费/酒有节假日季节性（春节/中秋备货），
但本质偏成长，检验其"季节锚"是否在训练期成立。
"""
...
CODES = {
    "512600": (2017, 2021, "主要消费(节假日)"),
    "512690": (2019, 2021, "酒(春节/中秋备货)"),
}


def load_code(code: str, y0: int, y1: int) -> pd.DataFrame:
    frames = []
...
def diagnose(code: str) -> None:
    y0, y1, name = CODES[code]
    df = load_code(code, y0, y1)
    ...
    if rows:
        print(f"回归中轨率: {len(hit)}/{len(rows)} ({len(hit)/len(rows)*100:.1f}%)")
