...
def initialize(context):
    ...
    # === 核心参数 ===
    context.max_stocks = 4
    context.single_max_pct = 0.25
    context.reserve_pct = 0.10
    context.per_buy_pct = 0.25
    context.top_n = 10
    
    # === 止损止盈 ===
    context.stop_loss_pct = 0.15
    context.max_loss_pct = 0.20
    context.take_profit_pct = 0.40
    context.take_profit_sell_pct = 0.30
    context.trailing_start = 0.35
    context.trailing_stop = 0.15
    context.partial_profit_dd = 0.15
    context.dynamic_stop_days = 30
    context.dynamic_stop_loss = 0.05
    ...
    # === 动态止损 ===
    context.bull_stop_loss = 0.20
    context.bear_stop_loss = 0.12
    context.bull_max_stocks = 4
    context.bear_max_stocks = 2
    context.bull_single_max = 0.25
    context.bear_single_max = 0.30
...
def before_trading_start(context, data):
    ...
    if context.last_pool_update_month != context.current_dt.month:
        try:
            # 性能优化：使用集合去重
            pool = set()
            pool.update(get_index_stocks('000300.SS'))
            pool.update(get_index_stocks('000905.SS'))
            pool.update(get_index_stocks('399006.SS'))
    ...
    # 市场转熊减仓
    if context.target_position < 0.7 and len(context.portfolio.positions) > 2:
        log.info(f'市场转熊（目标仓位{context.target_position*100:.0f}%），主动减仓至 2 只')
    ...
    if trade.is_closed:
        ...
        if trade.pnl > 0:
            ...
            if context.consecutive_losses == context.max_consecutive_losses:
                ...
                log.info(f'⚠️  冷却：连续{context.consecutive_losses}次亏损，停止{context.loss_cooldown_days}天')
                context.consecutive_losses = 0
                context.last_trade_was_loss = False
...
def get_fundamental_data(context, stock, fi=None):
    ...
    try:
        ...
        # 数据转换
        if rg and rg>1: rg/=100
        if npg and npg>1: npg/=100
        if roe:
            if roe>0.5: roe/=100
...
def check_market_state(context):
    """市场状态 - 性能优化：减少计算"""
    try:
        h = get_history(61, '1d', ['close'], security_list='000300.SS', fq='pre')
        if h and len(h)>=61:
            ...
            if len(c)>0:
                ...
                if context.market_strength >= 1.05:
                    ...
                    context.max_stocks, context.single_max_pct = 3, 0.25
                else:
...
def execute_buy(context, data, stock, tag, ind=None, pe=None, pb=None, rg=None, npg=None, roe=None):
    ...
    # 行业集中度检查
    if ind:
        iw = sum((context.portfolio.positions[s].last_sale_price*context.portfolio.positions[s].amount)/context.portfolio.total_value
    ...
    abv = min(pbv, ac)
    dp = i['close']
    bs = max((int(abv/dp/100)*100//100)*100, 100)
...
def execute_sell(context, data, stock, reason='', force=False):
    ...
    if ss>=ra: clean_stock_data(context, stock)
    return True

def select_stocks(context, data):
    """选股 - 性能优化：批量处理"""
    scored = []
    stocks = context.stock_pool if context.stock_pool else get_Ashares()
    tc = len(stocks)
    log.info(f'📊 选股：{tc}只')
    ...
    for i, s in enumerate(stocks):
        ...
        try:
            h = get_history(250, '1d', ['close','volume'], security_list=s, fq='pre')
            if h is None or len(h)<250: continue
            p = h['close'].values[-1]
            v = h['volume'].values[-1]
            if p<context.min_price or p>context.max_price or v<=0: continue
            ...
            if not (i['above_ma250'] and i['ma20_above_ma60'] and i['ma20_above_ma250']): continue
            if i['momentum_20']<0 or i['momentum_60']<context.min_momentum_60: continue
            if i['rsi14']>context.max_rsi: continue
            if not check_valuation(context, ind, pe, pb, rg, npg, roe): continue
            if not check_cash_flow(context, s): continue
            
            # 评分（性能优化：分离计算）
            ts = i['momentum_20']*100 + i['momentum_60']*50
            ts += (1 if i['above_ma250'] else 0)*20 + (1 if i['ma20_above_ma60'] else 0)*20 + (1 if i['ma20_above_ma250'] else 0)*20
            ts += i['vol_ratio']*10
            fs = (roe if roe else 0)*50*fw
            scored.append((s, ts+fs, fund))
        except:
            continue
    
    scored.sort(key=lambda x:x[1], reverse=True)
    log.info(f'✅ 选出{len(scored)}只')
    return [(type('obj',(object,),{'code':x[0],'close':calc_indicators(context,x[0])['close'] if calc_indicators(context,x[0]) else 0}), x[1], x[2]) for x in scored[:context.top_n]]
...
def in_cooldown(context):
    """冷却期"""
    return context.cooldown_end_date is not None and context.current_dt >= context.cooldown_end_date
...
def handle_data(context, data):
    ...
    # 选股
    ctx = select_stocks(context, data)
    if ctx:
        log.info(f'候选：{ctx[0][0].code}')
        s, sc, f = ctx[0]
        stock, score, fund = s.code, sc, f
        if stock not in context.portfolio.positions:
            ind, pe, pb, rg, npg, roe, gw, fw = fund
            rg_ = f'{rg:.2%}' if rg else 'N/A'
            npg_ = f'{npg:.2%}' if npg else 'N/A'
            roe_ = f'{roe:.2%}' if roe else 'N/A'
            log.info(f'【候选】{stock} 评分={score:.1f} 行业={ind} PE={pe} PB={pb} 营收={rg_} 净利={npg_} ROE={roe_}')
            context.last_signal_date[f'buy_{stock}'] = context.current_dt
            execute_buy(context, data, stock, '龙头', ind, pe, pb, rg, npg, roe)
