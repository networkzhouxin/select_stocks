# -*- coding: utf-8 -*-
"""
小牛量化选股策略 V2.0 - 基于V1回测结果的针对性优化
=============================================
V1回测结果：-47.6%总收益，52.9%最大回撤（2020-2025，10万起始）

V2优化点（基于V1的5个核心问题）：
  1. 仓位大幅提升：base_ratio从0.18-0.28提升至0.55-0.70（V1现金占73%）
  2. 移除dragon信号：V1中dragon占54%交易但盈亏比仅1.17，累计-105%
  3. 收紧形态止损：stop_price计算更紧（V1形态止损均亏-6.0%）
  4. 高量柱加权：V1唯一盈利信号（+60%），给予额外仓位奖励
  5. 时间止损缩短：10天→8天（V1时间止损胜率69%，是最佳出场方式）
  6. 定庄信号提高门槛：V1中lizhuang盈亏比2.15但胜率仅28%，要求score=3才入场

股票池：沪深800成分股（沪深300+中证500）中的主板股票（60/00开头）

核心信号源（3个主信号，移除dragon）：
  1. 高量柱（倍量柱实底实顶+价格收复实顶）         — 核心盈利信号
  2. 定庄起爆（立庄K线+缩量回调+放量突破）         — 仅score=3入场
  3. 均线粘合发散（5/10/20/60MA收敛后向上发散）    — 中期趋势信号
  4. 量价确认（缩量上涨/放量突破/缩量回调）        — 辅助加分项

风控：
  - ATR跟踪止损：2.0x ATR，夹紧3%-6%（V1为3%-8%）
  - 形态止损：收紧支撑位计算
  - 时间止损：8个交易日（V1为10天）
  - 单只上限35%仓位，最多3-4只

回测建议：同V1（2020-01-01 ~ 2025-12-31，100,000元）
"""
...
def initialize(context):
    ...
    g.params = {
        ...
        'stop_cap': 0.06,                # V2：止损上限6%（V1为8%，形态止损均亏-6%太大）
        'max_hold_days': 8,              # V2：时间止损8天（V1为10天，时间止损是最佳出场）
        'cooldown_days': 3,              # 冷却期3天
        'max_single_pct': 0.35,          # V2：单只最大35%（V1为30%，配合更少持仓数）

        # 评分门槛
        'min_composite_score': 2.5,      # V2：提高至2.5（V1为2.0，过滤低质量信号）
    }
...
def scan_and_rank(universe, prev_date, current_data):
    ...
    for code in universe:
        try:
            ...
            # 取最强形态信号
            signals = []
            # V2：定庄信号要求score=3才入场（V1胜率仅28%，需放量突破确认）
            if sig_lizhuang and sig_lizhuang['score'] >= 3:
...
def do_trading(context):
    ...
    # ======== 第五步：卖出持仓（V2：仅在超限时替换，不主动卖出未超限持仓）========
    ...
    for sig in candidates:
        ...
        # V2：高量柱信号额外仓位奖励（V1唯一盈利信号）
        if sig['best_pattern'] == 'high_vol':
            alloc *= 1.15

        # 单只上限35%（V2从30%放宽，配合更少持仓数）
