...
def observation_calc_rsi(close, period=14):
    """RSI formula copied from the reference resonance event definition."""
...
def observation_calc_kdj(high, low, close, n=9, m1=3, m2=3):
    """KDJ formula copied from the reference resonance event definition."""
...
def observation_calc_bollinger(close, period=20, std_mult=2.0):
    """Population-standard-deviation BOLL used by the reference strategy."""
    close = pd.Series(close, dtype=float)
...
def _observation_has_values(previous, current, fields):
...
def _detect_observation_rsi_turn(previous, current):
    if not _observation_has_values(previous, current, ("rsi14",)):
        return None
    if previous["rsi14"] <= 30.0 and current["rsi14"] > previous["rsi14"]:
        return "BUY"
    if previous["rsi14"] >= 70.0 and current["rsi14"] < previous["rsi14"]:
        return "SELL"
    return None


def _detect_observation_kdj_turn(previous, current):
    fields = ("k", "d", "j", "kd_diff")
    if not _observation_has_values(previous, current, fields):
        return None
    buy_extreme = min(previous["k"], previous["d"]) <= 20.0 or previous["j"] <= 0.0
    sell_extreme = max(previous["k"], previous["d"]) >= 80.0 or previous["j"] >= 100.0
...
def _apply_observation_event(active, indicator, direction, event_date, current):
    if direction is None:
        return
    reference_extreme = None
    if indicator == "BOLL":
        reference_extreme = current["low"] if direction == "BUY" else current["high"]
    active[indicator] = {
        "direction": direction,
...
def _invalidate_observation_boll(active, current):
    event = active.get("BOLL")
    if event is None:
        return
    if (event["direction"] == "BUY" and
            current["close"] <= current["boll_lower"] and
            current["low"] < event["reference_extreme"]):
        active.pop("BOLL", None)
    elif (event["direction"] == "SELL" and
          current["close"] >= current["boll_upper"] and
          current["high"] > event["reference_extreme"]):
        active.pop("BOLL", None)


def build_resonance_entry_observation_from_indicators(indicator_frame, signal_date):
    """Classify one Cross entry without changing any Cross decision field."""
    signal_text = _observation_date(signal_date)
    frame = indicator_frame.loc[
        [
            _observation_date(index_value) <= signal_text
            for index_value in indicator_frame.index
        ]
    ]
    if len(frame) < 3:
        raise ValueError("resonance observation requires at least three daily rows")
    if _observation_date(frame.index[-1]) != signal_text:
        raise ValueError("latest observation row must equal signal_date")

    active = {}
    first_event_position = max(1, len(frame) - 2)
    for position in range(first_event_position, len(frame)):
        previous = frame.iloc[position - 1]
        current = frame.iloc[position]
        event_date = _observation_date(frame.index[position])
        _apply_observation_event(
            active, "BOLL", _detect_observation_boll_turn(previous, current),
            event_date, current)
        _apply_observation_event(
            active, "RSI", _detect_observation_rsi_turn(previous, current),
            event_date, current)
        _apply_observation_event(
            active, "KDJ", _detect_observation_kdj_turn(previous, current),
            event_date, current)
        _invalidate_observation_boll(active, current)

    events = {}
    for indicator in ("BOLL", "RSI", "KDJ"):
        event = active.get(indicator)
        if event is None:
            continue
        events[indicator] = {
            "direction": event["direction"],
            "event_date": event["event_date"],
            "age": 0 if event["event_date"] == signal_text else 1,
        }
    ...
    supporters = tuple(
        ...
        if events.get(indicator, {}).get("direction") == "BUY"
    )
    fresh_supporters = tuple(
        indicator for indicator in supporters
        if events[indicator]["event_date"] == signal_text
    )

    if boll is None or boll["direction"] != "BUY":
        result = "BOLL_NOT_SUPPORTING"
        complete = False
    elif _builtins.any(
            event is not None and event["direction"] == "SELL"
...
def build_resonance_entry_observation(price_frame, signal_date):
    ...
    if price_frame is None or _builtins.any(name not in price_frame for name in required):
        raise ValueError("price frame is missing required resonance columns")
    frame = price_frame.loc[:, list(required)].copy()
    frame["rsi14"] = observation_calc_rsi(frame["close"], 14)
    k, d, j = observation_calc_kdj(
        frame["high"], frame["low"], frame["close"], 9, 3, 3)
    frame["k"], frame["d"], frame["j"] = k, d, j
    frame["kd_diff"] = k - d
    mid, upper, lower = observation_calc_bollinger(frame["close"], 20, 2.0)
    frame["boll_mid"], frame["boll_upper"], frame["boll_lower"] = mid, upper, lower
    return build_resonance_entry_observation_from_indicators(frame, signal_date)


def safe_build_resonance_entry_observation(price_frame, signal_date):
    try:
        return build_resonance_entry_observation(price_frame, signal_date)
...
def _format_observation_event(observation, indicator):
    event = (observation.get("events") or {}).get(indicator)
    if event is None:
        return "NONE"
    return "%s@%s:%s" % (
        event.get("direction", "NONE"),
        event.get("age", "NA"),
        event.get("event_date", "NA"),
    )


def emit_resonance_entry_observation(score, execution_date, signal_date):
    """Best-effort logging after a fill; this function never controls trading."""
    code = str(score.get("code", "UNKNOWN"))
    try:
        observation = score.get("resonance_entry_observation") or {
            "status": "MISSING", "result": "UNAVAILABLE",
            "is_complete": False,
        }
        status = observation.get("status", "MISSING")
        if status != "OK":
            log.info(
                "[resonance-entry-observation] schema=%s build=%s rule=%s code=%s "
                "execution_date=%s signal_date=%s status=%s result=%s error=%s" % (
                    RESONANCE_OBSERVATION_SCHEMA,
                    RESONANCE_OBSERVATION_BUILD_ID,
                    RESONANCE_OBSERVATION_RULE_ID,
                    code,
        ...
        log.info(
            "[resonance-entry-observation] schema=%s build=%s rule=%s code=%s "
            "execution_date=%s signal_date=%s status=OK result=%s complete=%s "
        ...
        try:
            log.warning(
                "[resonance-entry-observation-error] code=%s execution_date=%s "
    ...
    for score in candidates:
        ...
        if not has_position(context, code):
            log.warning(
                ...
                "lower-ranked candidates will not be promoted" % code)
            continue
        emit_resonance_entry_observation(score, today, prev_date)
