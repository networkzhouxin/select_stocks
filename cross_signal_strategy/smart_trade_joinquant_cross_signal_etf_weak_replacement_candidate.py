...
STRATEGY_VERSION = "cross-v0.3.2-weak-replacement-candidate"
...
def should_protect_weak_no_replacement_signal_sell(code, score, all_scores, held_codes, params=None):
    p = params or get_default_params()
    sell_score = _numeric_score(score.get("sell_score"))
    if sell_score < p["sell_threshold"] or sell_score >= 35:
        return False
    if _numeric_score(score.get("buy_score")) < 35:
        return False
    return not has_replacement_candidate_after_signal_sell(code, all_scores, held_codes, p)


def has_replacement_candidate_after_signal_sell(code, all_scores, held_codes, params=None):
    p = params or get_default_params()
    normalized_code = str(code).split(".")[0]
    held_if_sold = [
        str(item).split(".")[0]
        for item in held_codes
        if str(item).split(".")[0] != normalized_code
    ]
    candidates = filter_buy_candidates(all_scores, held_if_sold, p)
    return any(str(item.get("code")).split(".")[0] != normalized_code for item in candidates)
...
def do_trading(context):
    ...
    for code in list(held):
        ...
        if should_force_sell(score, False, p):
            if should_protect_weak_no_replacement_signal_sell(code, score, all_scores, held, p):
                log.info("[hold] %s weak sell_score %.0f no replacement, skip signal sell" % (
