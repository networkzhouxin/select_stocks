...
- 09:36 对 09:35 已提交且仍待确认的买卖订单执行一次 `get_trades()` 成交兜底；
...
PTrade 的 `on_trade_response` 是快速成交回报路径，但实盘中可能出现买卖订单已经成交而回调未到达策略，或者成交主推早于委托应答、暂时没有 `order_id` 的情况。策略因此在 09:36 对 09:35 委托、在 10:36 对 10:35 补偿委托分别通过官方 `get_trades()` 主动核对一次，只接受方向和订单号与当前待确认买单或卖单完全一致的成交。买入成交仍通过同一套成交状态机恢复实际成交数量、成交价格、买入日期、入场 ATR 和最高收盘价基线；不能精确匹配的记录不会被接管。任一时点没有待确认订单时直接返回，不调用 `get_trades()`。
...
`10:36主动核对` 和 `get_open_orders`。若进程重启后只能从券商未完成委托恢复，原提交时间
