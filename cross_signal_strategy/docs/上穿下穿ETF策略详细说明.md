...
买入时记录信号日的 ATR14，并以买入执行时的价格基准作为初始最高价；PTrade 在收到真实成交回报后再用成交事实校正持仓状态。此后只用已经确认完成的每日收盘价更新持仓最高价，不使用盘中最高价，以减少上影线噪声对止损线的错误收紧。PTrade 实盘在 `after_trading_end` 中取得的当日数值只作为盘后观察日志，不写入 `highest_since_buy`；下一个交易日的 `before_trading_start` 在恢复持仓状态后，按精确日期读取最终 T-1 日线。只有日期严格等于 T-1、收盘价为有限正数且成交量可证明的记录才能更新最高收盘价；成交量为 0 视为停牌日并保留原值，缺失、错日、格式异常或读取失败则把该仓位标记为未验证并闭锁自动卖出和新买入。这个时序只使用 T 日盘前已经完成的 T-1 数据，不构成未来函数。
