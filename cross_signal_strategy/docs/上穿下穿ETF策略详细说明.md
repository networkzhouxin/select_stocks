...
纯文字顺序：09:35 启动 → 确定 T-1 → ATR 止损 → 读取日线 → 计算指标 → 识别交叉 → 计算评分 → 普通信号卖出 → 重算仓位 → 筛选买入 → 下单 → 09:36 核对缺失的卖出成交回报并恢复延后补买 → 15:30 维护状态。
...
若全部待卖委托已确认完成，策略立即使用 09:35 已冻结的候选恢复补买；不会在 09:36 重新读取日线、重新计算指标或改变排名。若卖单仍未完成，则继续保留 10:35 的停牌、废单和部分成交补偿。该机制只修复订单生命周期，不改变策略信号与交易规则。
