...
PTrade 的 `on_trade_response` 是快速成交回报路径，但实盘中可能出现卖单已经成交而回调未到达策略的情况。策略因此在 09:36 通过官方 `get_trades()` 主动核对一次，只接受订单号与当前待确认卖单完全一致的卖出成交。
...
09:36 额外输出 `[订单核对汇总]`，记录待核对卖单、匹配成交和核对后未完成
数量；盘后输出 `[订单生命周期汇总]`，记录待买委托、待卖委托、延后买入和
委托状态未知保护。该诊断不新增任务，不改变订单匹配、成交累计、补买、
重试、现金、持仓或任何策略判断。
