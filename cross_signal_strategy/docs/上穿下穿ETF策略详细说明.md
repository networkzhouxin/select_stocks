...
```mermaid
flowchart TD
    A["T 日 09:35 启动"] --> B["确定上一交易日 T-1"]
    B --> C["先检查现有持仓 ATR 止损"]
    C --> D["读取每只 ETF 截至 T-1 的 120 根日线"]
    D --> E["计算 RSI、MACD、KDJ、BOLL、均线、ATR、DMI/ADX、成交量"]
    E --> F["识别最近 3 个交易日内的最新上穿/下穿方向"]
    F --> G["计算买入分、卖出分及位置/趋势/量能确认"]
    G --> H["先处理持仓的普通信号卖出"]
    H --> I["重新计算空余仓位"]
    I --> J["筛选并排序买入候选"]
    J --> K["按最多 3 只、总投入 95% 下单"]
    K --> L["15:30 用收盘价更新持仓最高价和止损线"]
```
