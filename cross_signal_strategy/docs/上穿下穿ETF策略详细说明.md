...
第一次实际买入评估会冻结当日原定候选。某只原定 ETF 下单异常、撤单或最终
零成交时，当日不再重试它，也不会晋升排名更靠后的 ETF；原本就为其他空余
仓位选中的候选仍可正常执行。09:35 确实停牌的 ETF 若在 10:35 恢复交易，可
按原始 T-1 排名加入同一冻结名单，属于停牌延迟执行，不属于后备补位。
