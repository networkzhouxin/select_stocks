...
### 7.4 PTrade 实盘买入报价与候选冻结

PTrade 实盘买单使用同一份已经通过新鲜度校验的盘口快照，以卖五价作为限价。
卖五档必须同时具有有限正价格和正数量，并且不得高于涨停价；盘口缺失、格式
异常、卖五无量或越过涨停价时闭锁该笔买入，不退回最新价或涨停价下单。限价
只限定可接受的最高成交价，交易所仍按价格优先原则撮合，因此实际成交价可以
低于卖五价，并不一定恰好等于卖五价。PTrade 回测无法证明真实五档盘口，只
保留当前价作为运行冒烟测试口径，收益仍以聚宽回测为准。

第一次实际买入评估先按原始 T-1 排名检查可交易性，再冻结当日原定候选。
只有明确停牌可以释放原定买入名额并顺位补位，下一只仍须通过全部既有买入条件；
若没有合格候选则保留现金。交易状态无法确认时闭锁该笔买入，但未知状态不等于
明确停牌，不能据此释放名额。报价不可用、卖五无量或格式异常、下单异常、未返回
委托编号、撤单、零成交或部分成交，都不会晋升更低排名的 ETF；原本就为其他
空余仓位选中的候选仍可正常执行。策略不单独把涨停作为补位理由。09:35 确实
停牌的 ETF 若在 10:35 恢复交易，只有组合仍有剩余仓位时才可按原始 T-1 排名
加入冻结名单，不会挤掉 09:35 已补买或已挂单的候选。
...
### 10.1 组合级 ATR 压力减仓（v0.3.3 新增）

策略维护一个组合级状态 `atr_stop_history`：每次 ATR 止损**足额成交**后记录该交易日的触发日期（信号卖出、废单、撤单和部分成交不计入）。每次提交新买单前，统计最近 **15 个交易日**内足额成交的 ATR 止损次数；若达到 **3 次及以上**，当日所有新买入目标缩小为标准目标的 **50%**，并在买入日志中输出 `stress=0.50`；否则输出 `stress=1.00`，即正常仓位。

- 该规则只影响**新买入的目标大小**，从不缩小已有持仓，也不改变任何买卖决策；
- 压力记忆随交易日自然滚动：止损事件滚出 15 日窗口后，下一笔新买入自动恢复满仓；
- 交易日历不可用时回退为满仓（fail-open，即回到 v0.3.2 行为）；
- 该状态随状态台账持久化并在重启后恢复；升级到 v0.3.3 后的首次启动，压力记忆冷启动为空（建议在非压力期部署）；
- 五个冻结验证窗口的结论：触发窗口（训练期、2022-2023）收益与回撤同向改善，未触发窗口路径与 v0.3.2 完全一致，无副作用。
...
PTrade 的 `on_trade_response` 是快速成交回报路径，但实盘中可能出现买卖订单已经成交而回调未到达策略，或者成交主推早于委托应答、暂时没有 `order_id` 的情况。策略因此在 09:36 对 09:35 委托、在 10:35 补偿流程刷新开放委托之前、以及在 10:36 对 10:35 补偿委托通过官方 `get_trades()` 主动核对，只接受方向和订单号与当前待确认买单或卖单完全一致的成交。对于仍没有匹配成交记录的确切待确认订单，同一任务再调用 `get_order(order_id)`；只有返回订单号、证券代码和累计数量均可核验，且状态为 `5`、`6` 或 `9` 时，才接纳该终态。已证明的零成交失败买单只释放该笔订单的待确认状态，并把原定代码标记为当日失败，不晋升后排候选；已证明的零成交失败卖单恢复为可重试，同时保留原持仓风险状态。若 `get_order` 返回状态 `8`（全部成交），策略会明确记录“等待成交明细”，但不会单凭委托对象推断成交价格或成交额，也不会提前释放待确认状态；后续仍由真实成交主推或 `get_trades()` 明细进入既有成交状态机。查询失败、返回矛盾、数量异常或其他非终态时继续保持待确认并闭锁。买入成交仍通过同一套成交状态机恢复实际成交数量、成交价格、买入日期、入场 ATR 和最高收盘价基线；不能精确匹配的记录不会被接管。任一时点没有待确认订单时直接返回，不调用 `get_trades()` 或 `get_order(order_id)`。
...
6. 对 QDII 额外观察停牌、溢折价和成交可得性；买入端 5% IOPV 分类仍只观察、不改变委托。PTrade 实盘卖出端另有一条 8% 执行覆盖：持有满 5 个交易日、T-1 卖出分达到 30，且原信号卖出仅被价格确认或 ADX 阻止时，如果有效买一价相对 IOPV 溢价达到 8%，直接整仓卖出；行情非新鲜、停牌、IOPV/买一无效或非实盘时不触发。
