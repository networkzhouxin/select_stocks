...
Implementation classification: `invalid_implementation`. The executable first-run implementation used sell RSI 12 instead of 10, sell MACD 5 instead of 4, and allowed ADX to suppress an ordinary raw sell conflict before buy eligibility, so it did not instantiate the approved frozen rule.
Training diagnostic result: Invalid-implementation provenance only. That run changed 196 filled-order days across every training year (2019: 62, 2020: 64, 2021: 70) and closed trades retained 85 of the baseline 89 (95.51%). These figures cannot establish materiality or trade retention for the corrected approved rule.
Candidate result: Not determined for the approved rule. The invalid implementation observed nominal total return +78.13% versus baseline +125.00%, annualized return 21.29% versus 31.13%, maximum drawdown 6.37% versus 6.03%, win rate 51.76% versus 56.18%, Sharpe 1.672 versus 2.262, Sortino 2.533 versus 3.581, and profit/loss ratio 2.831 versus 4.878. Under doubled friction, the invalid implementation observed return +63.32% versus +108.15%, win rate 45.88% versus 51.69%, and profit/loss ratio 2.347 versus 3.966. These observations are retained only to preserve provenance and do not approve or reject v0.4.
JoinQuant training result: Not run. No JoinQuant or PTrade candidate was generated; both formal strategy files remain unchanged.
...
Why it failed: The first run is invalid as an implementation of the approved experiment, regardless of its observed gate values. The implementation deviations alter sell-score totals and buy eligibility, so its order path and results cannot answer the pre-registered hypothesis.
Can it be revisited? yes, exactly once as the approved implementation-correction replay
Conditions for revisiting: Run only the same approved rule after correcting sell RSI 12→10, sell MACD 5→4, and ordinary raw buy-side sell-conflict handling. Keep every other score, cap, floor, threshold, indicator period, ETF, friction, hold, risk, ranking, and portfolio rule frozen. Do not create alternatives, read validation data, or modify JoinQuant/PTrade before the corrective local gate.

Date: 2026-08-23
Version: `cross-v0.4.0-dimension-capped-candidate` (corrected local research replay)
Experiment: Execute the sole authorized implementation-correction replay of the exact approved v0.4 dimension-capped rule after fixing only sell RSI 12 to 10, sell MACD 5 to 4, and raw buy-side sell-conflict blocking independent of ADX and holding state. Keep the approved rule fingerprint `0493e7fbeb80cdaa6d8ab0fe9c47d3fa8ca8b680e6556ca805de4d6e742f7f63`, nine-ETF pool, 40/24/18 gates, indicator periods, ranking, five-session hold, ATR path, 09:35/T-1 boundary, portfolio rules, nominal friction, doubled friction, and 2019-2021 training window frozen.
Hypothesis: Correctly capping correlated indicator dimensions and requiring independent reversal, location, and trend evidence may improve closed-trade accuracy without materially weakening return, drawdown, payoff quality, trade count, or doubled-friction robustness.
Training diagnostic result: The corrected replay changed 196 filled-order days across every training year (2019: 62, 2020: 64, 2021: 70). It retained 85 of 89 baseline closed trades (95.51%). Nominal and doubled-friction candidate score-attempt audits each reconciled 6,570 attempts (6,111 scored, 459 skipped); each execution audit reconciled 186 planned orders and 173 fills against its own replay and performance counts.
Candidate result: Rejected by seven frozen local gates. Nominal baseline/candidate results were +125.00%/+78.13% total return, 31.13%/21.29% annualized return, 6.03%/6.37% maximum drawdown, 56.18%/51.76% win rate, 2.262/1.672 Sharpe, 3.581/2.533 Sortino, and 4.878/2.831 profit/loss ratio. Buys/sells/closed trades were 92/89/89 versus 88/85/85. Annual returns were +35.84%/+52.68%/+8.49% versus +21.45%/+43.89%/+1.93%.
Doubled-friction result: Baseline/candidate results were +108.15%/+63.32% total return, 27.77%/17.82% annualized return, 6.39%/6.93% maximum drawdown, 51.69%/45.88% win rate, 2.039/1.422 Sharpe, 3.186/2.125 Sortino, and 3.966/2.347 profit/loss ratio. Candidate annual returns were +18.61%/+39.44%/-1.26%.
Gate result: `STOP`. Candidate win rate did not strictly improve; nominal return, Sharpe, Sortino, and profit/loss ratio each retained less than 95% of baseline; doubled-friction return retained less than 95%; and doubled-friction win rate was below baseline.
Evidence: Canonical corrected report `cross_signal_strategy/reports/dimension_capped_score_v04_2019_2021.md`, 9,740,215 bytes, SHA-256 `14395dbf09f506c914bd5da241454b3af4291cf2e701e7653738f50f4840ccd6`. The invalid-implementation report remains separately preserved byte-for-byte and is not substituted for this result.
JoinQuant training result: Not run because the corrected local gate failed. No JoinQuant or PTrade candidate was created; both formal strategies remain unchanged.
Validation result: Not run. No reserved validation, pressure, recent/live, or full-period data was read or used.
Why it failed: The corrected approved score architecture materially changed the order path but reduced rather than improved trade accuracy and payoff quality. Independent dimension floors did not compensate for the lost additive signal strength, and the candidate remained substantially more fragile under doubled friction. The frozen accuracy-first and retention gates therefore reject the rule without authorizing any follow-up variant.
Can it be revisited? no; the sole corrective replay is consumed and the v0.4 dimension-capped family is exhausted
Conditions for revisiting: Do not search nearby score weights, caps, floors, 39/41 buy gates, 23/25 sell gates, indicator deletion, alternate ranking, ADX/protection changes, hold periods, ATR rules, pool changes, friction changes, ETF/year exceptions, or validation-period repairs. Reopening requires a genuinely independent market-structure hypothesis, new pre-registration, and explicit user authorization.

Date: 2026-08-25
Version: `krba-v0.1-candidate` (independent local research candidate only)
Experiment: Test the single frozen KDJ-timing + RSI-position + BOLL-structure + ATR-risk strategy. Buy only on an exact T-1 K/D bullish cross with RSI6 at or below 30 and a T-1 lower-band rejection (`low <= BOLL lower` and `close > BOLL lower`). Rank only eligible candidates by K-D, then RSI and fixed pool order. Keep at most three equal slots using 95% total exposure. Sell at 09:35 by frozen-entry-ATR stop, BOLL upper target after five sessions, or post-midline KDJ/BOLL weakness after five sessions; add a 14:50 ATR-only check using that minute's open and no later data. Preserve the formal nine-ETF pool, 2018 warm-up/2019-2021 training boundary, nominal costs, and one corrected doubled-friction replay.
Hypothesis: Separating indicator duties may avoid lagging MACD entries and delayed multi-condition exits, improving closed-trade accuracy while retaining enough opportunities, return, payoff quality, and friction robustness to justify an independent JoinQuant candidate.
Training diagnostic result: The same-mouth local baseline/candidate results were +125.00%/+6.09% total return, 31.13%/2.00% annualized return, 6.03%/6.21% maximum drawdown, 56.18%/85.71% win rate, and 4.878/4.091 profit/loss ratio. Baseline/candidate buys, sells, and closed trades were 92/89/89 versus 7/7/7. Candidate annual returns were 0.00% in 2019, +4.00% in 2020, and +2.02% in 2021; closed trades were 0/5/2. Its largest winning trade contributed 28.34% of candidate gross profit.
Doubled-friction result: The first arm is classified `invalid_implementation` because it raised commission and slippage but incorrectly retained the nominal 5-yuan minimum commission. A test-first correction changed only the minimum commission to 10 yuan. The corrected baseline/candidate results were +108.15%/+5.25% total return, 6.39%/6.51% maximum drawdown, 51.69%/85.71% win rate, and 3.966/3.517 profit/loss ratio.
Candidate result: Rejected with `STOP`. It failed the frozen minimum-50-closed-trades gate, the 80% nominal-return-retention gate, the every-training-year-activity gate, and the 75% doubled-friction-return-retention gate. Passing the raw win-rate and concentration checks does not override these primary failures.
JoinQuant training result: Not run because the frozen local gate failed. No standalone JoinQuant or PTrade candidate was generated; both formal strategies remain unchanged.
Validation result: Not run. No reserved validation, pressure, recent/live, or full-period data was read or used.
Why it failed: The conjunctive reversal setup is too sparse. Seven trades and a zero-trade year make the 85.71% win rate statistically and operationally misleading, while prolonged cash exposure removes almost all of the baseline return. The simple structure prevented delayed entries, but mostly by not entering rather than by identifying enough better entries.
Can it be revisited? no as this KDJ/RSI/BOLL/ATR mean-reversion family or a neighboring threshold/time variant
Conditions for revisiting: Keep the formal `cross-v0.3.3` mainline. Do not search nearby RSI levels, KDJ crossing definitions or retention, BOLL bands/reclaim rules, ATR periods/multipliers/floors, five-day hold, 14:50 timing, ETF/year exceptions, pool changes, ranking, or position sizing from this result. Reopening requires a genuinely independent prospective feature, new pre-registration, and explicit user authorization.
Data boundary: approved read-only 2018 warm-up plus 2019-2021 training data only; validation influence none
