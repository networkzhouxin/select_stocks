...
| `late_veto_early_pre_macd_user_authorized` | exhausted | The frozen stacked candidate failed the official 2019-2021 nominal JoinQuant gate: return fell 129.2480%→97.6525%, maximum drawdown rose 6.2838%→6.7473%, and closed-trade win rate fell 55.7895%→51.4851% instead of improving by at least 3 percentage points. Its 20 early fills covered all three years, but the 19 closed early-channel trades had only 6 winners and 13 losers. Complete logs and performance screenshots support rejection; independent trade exports were not provided, so the evidence is explicitly rejection-only and cannot support a pass or adoption. Doubled friction, validation, and full-period runs were not performed. No nearby variant is allowed. |
...
authorized `late_veto_early_pre_macd_user_authorized` stacked candidate also
completed the nominal 2019-2021 pair and was rejected: return and win rate fell
while maximum drawdown rose. Its rejection-only evidence mode records complete
logs and JoinQuant performance screenshots but no independent trade exports;
that exception cannot pass or adopt a candidate. The failed nominal gate stops
doubled friction, validation, and full-period runs. The result does not reopen
threshold, age, MACD, BOLL, RSI, ETF, queue, sell, or nearby-rule searches.
