...
### Prepare Frozen Signal-Only Exit Candidate

- Date: 2026-09-02.
- Hypothesis: Temporarily suppressing executed ATR exits may avoid stop-and-rebound churn and improve closed-trade win rate, but it qualifies only if total return does not decrease and maximum drawdown does not increase.
- Frozen change: The isolated `cross-v0.3.3-no-atr-exit-candidate` keeps computing the official ATR stop hit at the T-day execution price, emits `[atr-exit-observation]`, and takes no ATR order or actual-stop-history action. The existing T-1 `sell_score` threshold, confirmation, ADX protection, five-trading-day minimum hold, and full-position sell remain the only actual exit path.
- Baseline evidence: The user-supplied ordinary-friction JoinQuant log for the unchanged 2019-2021 path independently reconciles to +129.248% total return, 6.2838% maximum drawdown, and 53/42 closed trades (55.79% win rate). Its 27 ATR exits (17/10, +16309.3 CNY fixed-path net PnL) are attribution only and are not claimed as the result of this candidate.
- Pre-registered acceptance gate: On the same 2019-2021 JoinQuant training settings, closed-trade win rate must be greater than 55.79%, total return must be at least 129.25%, and maximum drawdown must be no more than 6.28%. Report trade count, annual returns, profit concentration, ending open positions, and a same-build doubled-friction A/B before any adoption decision. No nearby ATR mode, delayed stop, per-ETF exception, sell threshold, or confirmation search follows a failure.
- Research status: Candidate implementation and local code-path tests only; JoinQuant performance is pending. No reserved validation-period market data was read.
- Business boundary: Formal JoinQuant, formal PTrade, resonance observation, entry logic, scores, ranking, position sizing, pool, parameters, execution schedule, and T-1 signal data are unchanged. The production multi-factor strategy is untouched.
