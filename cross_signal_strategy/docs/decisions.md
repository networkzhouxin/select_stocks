...
Reason: Repeated 2019-2021 experiments create strategy-selection bias even when every individual experiment obeys the no-validation rule. After the 2026-07-14 controlled-breakout observation, the retained ledger can prove a minimum of 49 failed or non-adopted experiments plus the selected mainline, so the apparent training significance must be judged against at least 50 trials.
Evidence: The frozen local path returned +120.61% with 30.27% annualized return and 2.172 annualized Sharpe. Its single-trial PSR p-value is 0.000123988; the minimum-50 Bonferroni value is 0.00619941. A Newey-West/HAC mean-return test with automatic lag 6 gives a single-trial p-value of 0.0000622008 and minimum-50 Bonferroni value of 0.00311004. The PSR approximation remains below 5% through 403 trials.
Interpretation: The retained evidence is stronger than a marginal best-of-50 result, but the corrected confidence is an optimistic upper bound because 50 is only a provable minimum. This remains in-sample evidence and is not out-of-sample validation.
...
Affected files: `cross_signal_strategy/breakout_extension_diagnostics.py`, `tests/test_cross_signal_breakout_extension_diagnostics.py`, `cross_signal_strategy/docs/research_budget.json`, `cross_signal_strategy/docs/research_budget.md`, `tests/test_cross_signal_research_budget.py`, `cross_signal_strategy/docs/failed_experiments.md`, `cross_signal_strategy/docs/backtest_notes.md`, `cross_signal_strategy/docs/multiple_testing_audit.md`, `tests/test_cross_signal_multiple_testing_audit.py`, `cross_signal_strategy/docs/decisions.md`, `cross_signal_strategy/README.md`
Allowed validation influence: none; only approved 2018 warm-up and 2019-2021 training data were read
