# -*- coding: utf-8 -*-
"""
Cross-Signal ETF Strategy v0.3.3 no-ATR-exit candidate for JoinQuant.

Candidate contract:
- ATR exit conditions are observed but never submit an order.
- Actual exits remain exclusively on the unchanged sell-score path.
- Entry, ranking, sizing, pool, thresholds, and T-1 signal data are unchanged.
...
STRATEGY_VERSION = "cross-v0.3.3-no-atr-exit-candidate"
DEPLOYMENT_BUILD_ID = "20260902.3-candidate"
...
def handle_atr_stop_hits(context, stop_hits):
    """Apply only the action named by the candidate ATR exit policy."""
    for code, stop_price, price in stop_hits:
        if ATR_EXIT_POLICY == "EXECUTE":
            execute_sell(
                code, context,
                "atr_stop %.3f<=%.3f" % (price, stop_price),
            )
        elif ATR_EXIT_POLICY == "OBSERVE_ONLY":
            log.info(
                "[atr-exit-observation] %s policy=%s current=%.3f stop=%.3f "
                "action=hold sell_signal_only=True" % (
                    code, ATR_EXIT_POLICY, price, stop_price,
                ))
        else:
            raise ValueError("unsupported ATR_EXIT_POLICY: %s" % ATR_EXIT_POLICY)
...
def do_trading(context):
    ...
    handle_atr_stop_hits(context, stop_hits)
