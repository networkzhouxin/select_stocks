...
- `archive/probes/smart_trade_joinquant_underlying_availability_probe.py`: temporary no-order 2019-2021 JoinQuant probe that discovers the tracked index from official fund metadata, checks T-1 readability at 09:35, and runs a same-day future-data negative control. It establishes only platform readability and cannot establish the publisher's original release timestamp.
