...
Temporary probe copy:
- Use only to diagnose 513880.XSHG tradability on 2019-12-12.
- Do not treat this file as a production strategy version.
...
def probe_513880_status(context):
    ...
    log.info(
        "[probe-513880-status] dt=%s code=%s paused=%s last_price=%s day_open=%s high_limit=%s low_limit=%s",
        context.current_dt,
        code,
        getattr(data, "paused", None),
        getattr(data, "last_price", None),
        getattr(data, "day_open", None),
        getattr(data, "high_limit", None),
        getattr(data, "low_limit", None),
    )
    ...
    try:
        ...
        log.info("[probe-513880-1m] dt=%s code=%s bars=%s", context.current_dt, code, bars)
