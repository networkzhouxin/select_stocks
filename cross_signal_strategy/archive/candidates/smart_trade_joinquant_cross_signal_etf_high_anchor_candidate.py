# -*- coding: utf-8 -*-
"""
Cross-Signal ETF Strategy high-anchor candidate for JoinQuant (isolated).

Training-only candidate for the user-authorized fixed intraday-high trailing
anchor experiment (2026-08-16). Only intended change versus the official
cross-v0.3.3 mainline: the trailing-high anchor is updated with each session's
intraday HIGH instead of its close. Multiplier 2.5, floor 5%, cap 15%, and
entry ATR are unchanged. Everything else is unchanged.
See docs/superpowers/specs/2026-08-16-intraday-high-anchor-design.md.
"""
...
STRATEGY_VERSION = "cross-v0.3.3-high-anchor-candidate"
...
def after_close(context):
    ...
    for code in holds:
        ...
        if price is None or price <= 0:
            continue
        try:
            day_high = float(current_data[code].high_price)
        except Exception:
            day_high = price
        anchor_value = day_high if day_high > 0 else price
        prev_high = g.highest_since_buy.get(code, anchor_value)
        g.highest_since_buy[code] = max(prev_high, anchor_value)
