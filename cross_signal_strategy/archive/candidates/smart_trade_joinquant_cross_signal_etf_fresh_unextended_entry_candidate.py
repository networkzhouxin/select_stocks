# -*- coding: utf-8 -*-
"""
Cross-Signal ETF Strategy v0.3.3 fresh-unextended entry candidate for JoinQuant.
...
STRATEGY_VERSION = "cross-v0.3.3-fresh-unextended-entry-candidate"
DEPLOYMENT_BUILD_ID = "20260822.1-candidate"
...
def filter_fresh_unextended_buy_candidates(scores, held_codes, params=None):
    p = params or get_default_params()
    held = set(held_codes)
    candidates = []
    for raw_score in scores:
        ...
        reversal_score = _numeric_score(score.get("reversal_score"))
        age = score.get("fresh_entry_earliest_cross_age")
        extension = score.get("fresh_entry_extension_atr")
        try:
            age = int(age)
            extension = float(extension)
        except (TypeError, ValueError):
            continue
        if not (FRESH_BUY_MIN_SCORE <= buy_score < FRESH_BUY_MAX_SCORE):
            continue
        if reversal_score < FRESH_MIN_REVERSAL_SCORE:
            continue
        if score.get("fresh_entry_context_complete") is not True:
            continue
        if age < 0 or age > FRESH_MAX_CROSS_AGE:
            continue
        if pd.isna(extension) or extension > FRESH_MAX_EXTENSION_ATR:
            continue
        if not score.get("buy_allowed"):
            continue
        if score.get("sell_score", 0) >= p["sell_threshold"]:
            continue
        if not has_new_buy_position(score, p):
            continue
        if is_blocked_entry_combo(score):
            continue
        if score.get("code") in held:
            continue
...
def build_new_buy_queue(scores, held_codes, params=None):
    """Keep the official primary queue first, then append the frozen fast path."""
    p = params or get_default_params()
    primary = [
        dict(score, entry_channel="primary")
        for score in filter_buy_candidates(scores, held_codes, p)
    ]
    fresh = [
        dict(score, entry_channel="fresh_unextended")
        for score in filter_fresh_unextended_buy_candidates(scores, held_codes, p)
    ]
...
def _candidate_contributing_bullish_cross_ages(snapshot):
    rsi_up = bool(
        snapshot.get("rsi6_cross_rsi12_up") or
        snapshot.get("rsi6_cross_rsi24_up"))
    rsi_down = bool(
    ...
    for field in fields:
        ...
        if field.startswith("rsi6_") and not (rsi_up and not rsi_down):
            continue
        raw_age = snapshot.get(field + "_age")
        try:
            age = int(raw_age)
        except (TypeError, ValueError):
            complete = False
...
def enrich_fresh_entry_snapshot(snapshot, closes):
    """Attach the frozen fast-entry context from already-visible daily closes."""
    result = dict(snapshot)
    ages, context_complete = _candidate_contributing_bullish_cross_ages(result)
    if not ages:
        result.update({
        ...
        return result
    earliest_age = max(ages)
    values = _as_float_array(closes)
    row_index = len(values) - 1 - earliest_age
    if row_index < 0:
        result.update({
            "fresh_entry_context_complete": False,
            "fresh_entry_earliest_cross_age": earliest_age,
            "fresh_entry_cross_close": None,
            "fresh_entry_extension_atr": None,
        })
        return result
    cross_close = float(values[row_index])
    close = result.get("close")
    atr = result.get("atr")
    extension = None
    if not _builtins.any(pd.isna(v) for v in [close, atr, cross_close]) \
            and float(atr) > 0 and cross_close > 0:
        extension = (float(close) - cross_close) / float(atr)
    result.update({
        "fresh_entry_context_complete": context_complete,
        "fresh_entry_earliest_cross_age": earliest_age,
...
def build_signal_snapshot(df, params):
    ...
    return enrich_fresh_entry_snapshot(snapshot, C)
...
def do_trading(context):
    ...
    if not candidates:
        log.info("[%s] no primary or fresh-unextended buy candidates" % STRATEGY_VERSION)
        return
    ...
    for score in candidates:
        ...
        log.info("[buy] %s channel=%s buy=%.0f rev=%.0f loc=%.0f trend=%.0f vol=%.0f "
                 "fresh_age=%s fresh_ext_atr=%s stress=%.2f target=%.0f" % (
            code, score.get("entry_channel", "primary"),
            score["buy_score"], score["reversal_score"], score["location_score"],
            score["trend_score"], score["volume_score"],
            score.get("fresh_entry_earliest_cross_age"),
            score.get("fresh_entry_extension_atr"), stress_scale, target_value))
