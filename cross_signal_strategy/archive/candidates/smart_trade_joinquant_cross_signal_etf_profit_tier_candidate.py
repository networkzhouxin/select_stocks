# -*- coding: utf-8 -*-
"""
Cross-Signal ETF Strategy profit-tier candidate for JoinQuant (isolated).

Training-only candidate for the user-authorized fixed profit-tiered ATR
tightening experiment (2026-08-16). Only intended change versus the official
cross-v0.3.3 mainline: the trailing ATR multiplier tightens by current profit
(x0.8 above 5% profit, x0.6 above 15% profit). Everything else is unchanged.
See docs/superpowers/specs/2026-08-16-profit-tiered-atr-design.md.
"""
...
STRATEGY_VERSION = "cross-v0.3.3-profit-tier-candidate"
...
def get_default_params():
    return {
        ...
        "profit_tier_low": 0.05,
        "profit_tier_high": 0.15,
        "profit_tier_factor_low": 0.8,
        "profit_tier_factor_high": 0.6,
    }
...
def calc_stop_price(highest, atr_val, cost, params=None, profit_pct=None):
    p = params or get_default_params()
    if highest <= 0 or atr_val <= 0:
        return cost * (1 - p["stop_cap"])
    mult = float(p["trailing_atr_mult"])
    if profit_pct is not None and profit_pct > 0:
        if profit_pct > p["profit_tier_high"]:
            mult *= float(p["profit_tier_factor_high"])
        elif profit_pct > p["profit_tier_low"]:
            mult *= float(p["profit_tier_factor_low"])
...
def check_atr_stops(context, current_data):
    ...
    for code in current_hold_codes(context):
        ...
        stop_price = calc_stop_price(
            g.highest_since_buy[code], g.entry_atr[code], pos.avg_cost, g.params,
            profit_pct=(price / pos.avg_cost - 1.0) if pos.avg_cost > 0 else None)
        if price <= stop_price:
...
def after_close(context):
    ...
    for code in holds:
        ...
        stop_price = calc_stop_price(
            g.highest_since_buy[code], atr_val, pos.avg_cost, g.params,
            profit_pct=(price / pos.avg_cost - 1.0) if pos.avg_cost > 0 else None
