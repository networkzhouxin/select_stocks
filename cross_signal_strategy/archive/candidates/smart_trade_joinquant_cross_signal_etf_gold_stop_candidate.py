# -*- coding: utf-8 -*-
"""
Cross-Signal ETF Strategy gold-stop candidate for JoinQuant (isolated).

Training-only candidate for the user-authorized fixed gold-specific stop
experiment (2026-08-16). Only intended change versus the official
cross-v0.3.3 mainline: 518880 uses stop_floor 0.03 and trailing_atr_mult 2.0
instead of 5%/2.5x. Everything else is unchanged.
See docs/superpowers/specs/2026-08-16-gold-specific-stop-design.md.
"""
...
STRATEGY_VERSION = "cross-v0.3.3-gold-stop-candidate"
...
def get_default_params():
    return {
        ...
        "gold_stop_floor": 0.03,
        "gold_trailing_atr_mult": 2.0,
    }
...
def calc_stop_price(code, highest, atr_val, cost, params=None):
    p = params or get_default_params()
    if highest <= 0 or atr_val <= 0:
        return cost * (1 - p["stop_cap"])
    code_base = str(code).split(".")[0]
    if code_base == "518880":
        floor = float(p["gold_stop_floor"])
        mult = float(p["gold_trailing_atr_mult"])
    else:
        floor = float(p["stop_floor"])
        mult = float(p["trailing_atr_mult"])
    pct_stop = mult * atr_val / highest
    pct_stop = max(floor, min(p["stop_cap"], pct_stop))
...
def check_atr_stops(context, current_data):
    ...
    for code in current_hold_codes(context):
        ...
        if code not in g.highest_since_buy or code not in g.entry_atr:
            continue
        stop_price = calc_stop_price(code, g.highest_since_buy[code], g.entry_atr[code], pos.avg_cost, g.params)
        if price <= stop_price:
...
def after_close(context):
    ...
    for code in holds:
        ...
        stop_price = calc_stop_price(code, g.highest_since_buy[code], atr_val, pos.avg_cost, g.params) \
