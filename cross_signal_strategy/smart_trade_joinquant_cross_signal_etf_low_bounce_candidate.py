...
STRATEGY_VERSION = "cross-v0.3.2-low-bounce-candidate"
...
def is_blocked_entry_combo(score):
    ...
    low_bounce_without_macd = (
        bool(rsi_up)
        and bool(kdj_up)
        and not bool(score.get("macd_cross_up"))
        and bool(score.get("close_between_boll_lower_mid"))
        and bool(score.get("close_near_ma20"))
        and _numeric_score(score.get("volume_score")) > 0
        and 0 < _numeric_score(score.get("trend_score")) < 20
    )
    return rsi_macd_without_kdj or low_bounce_without_macd
