# -*- coding: utf-8 -*-
"""
Cross-Signal ETF Strategy v0.3.3 stacked entry candidate for JoinQuant.

Research protocol:
- Develop first on 2019-01-01 to 2021-12-31 only.
- Use T-1 daily bars for all signals.
- Preserve the late-MACD/BOLL-upper veto on the primary >=60 channel.
- Let a frozen 50-59 pre-MACD channel fill only slots left by the primary queue.
...
STRATEGY_VERSION = "cross-v0.3.3-late-veto-early-pre-macd-candidate"
DEPLOYMENT_BUILD_ID = "20260822.3-candidate"

EARLY_BUY_MIN_SCORE = 50.0
EARLY_BUY_MAX_SCORE = 60.0
EARLY_MAX_CROSS_AGE = 1
...
def _active_prior_cross_age(score, flags):
    ages = []
    for flag in flags:
        if not bool(score.get(flag)):
            continue
        age = score.get("%s_age" % flag)
        if isinstance(age, bool) or age not in (1, 2):
            continue
        ages.append(int(age))
    return min(ages) if ages else None


def _active_early_cross_age(score, flags):
    ages = []
    for flag in flags:
        if not bool(score.get(flag)):
            continue
        age = score.get("%s_age" % flag)
        if isinstance(age, bool) or age not in range(EARLY_MAX_CROSS_AGE + 1):
            continue
        ages.append(int(age))
    return min(ages) if ages else None


def is_late_macd_boll_upper_entry(score):
    if not bool(score.get("macd_cross_up")):
        return False
    macd_age = score.get("macd_cross_up_age")
    if isinstance(macd_age, bool) or macd_age != 0:
        return False
    if rsi_group_direction(score) != "up":
        return False
    rsi_age = _active_prior_cross_age(score, (
        "rsi6_cross_rsi12_up",
        "rsi6_cross_rsi24_up",
    ))
    kdj_age = _active_prior_cross_age(score, (
        "kdj_k_cross_up",
        "kdj_j_cross_up",
    ))
    if rsi_age is None or kdj_age is None:
        return False
    try:
        close = float(score.get("close"))
        upper = float(score.get("boll_upper"))
    except (TypeError, ValueError):
        return False
    return bool(np.isfinite(close) and np.isfinite(upper) and close >= upper)


def is_early_pre_macd_entry(score, params=None):
    """Match the single frozen early-entry rule without relaxing primary buys."""
    p = params or get_default_params()
    buy_score = _numeric_score(score.get("buy_score"))
    if not EARLY_BUY_MIN_SCORE <= buy_score < EARLY_BUY_MAX_SCORE:
        return False
    if not bool(score.get("buy_allowed")):
        return False
    if rsi_group_direction(score) != "up":
        return False
    rsi_age = _active_early_cross_age(score, (
        "rsi6_cross_rsi12_up",
        "rsi6_cross_rsi24_up",
    ))
    kdj_age = _active_early_cross_age(score, (
        "kdj_k_cross_up",
        "kdj_j_cross_up",
    ))
    if rsi_age is None or kdj_age is None:
        return False
    if bool(score.get("macd_cross_up")):
        return False
    try:
        dif = float(score.get("dif"))
        dea = float(score.get("dea"))
        dif_prev = float(score.get("dif_prev"))
        dea_prev = float(score.get("dea_prev"))
        close = float(score.get("close"))
        upper = float(score.get("boll_upper"))
        rsi6 = float(score.get("rsi6"))
    except (TypeError, ValueError):
        return False
    values = (dif, dea, dif_prev, dea_prev, close, upper, rsi6)
    if not _builtins.all(np.isfinite(value) for value in values):
        return False
    macd_diff = dif - dea
    previous_macd_diff = dif_prev - dea_prev
    return bool(
        macd_diff < 0
        and macd_diff > previous_macd_diff
        and close < upper
        and rsi6 < float(p["overheat_rsi"])
    )


def is_official_new_buy_candidate(score, held_codes, params=None):
    p = params or get_default_params()
    held = set(held_codes)
    return (
        score.get("buy_allowed")
        and score.get("buy_score", 0) >= p["buy_threshold"]
        and score.get("sell_score", 0) < p["sell_threshold"]
        and has_new_buy_position(score, p)
        and not is_blocked_entry_combo(score)
        and score.get("code") not in held
    )
...
def filter_buy_candidates(scores, held_codes, params=None):
    ...
    return [
        s for s in scores
        if is_official_new_buy_candidate(s, held_codes, p)
        and not is_late_macd_boll_upper_entry(s)
    ]


def is_early_new_buy_candidate(score, held_codes, params=None):
    p = params or get_default_params()
    held = set(held_codes)
    return (
        is_early_pre_macd_entry(score, p)
        and score.get("sell_score", 0) < p["sell_threshold"]
        and has_new_buy_position(score, p)
        and not is_blocked_entry_combo(score)
        and score.get("code") not in held
...
def filter_early_pre_macd_buy_candidates(scores, held_codes, params=None):
    p = params or get_default_params()
    return [
        score for score in scores
        if is_early_new_buy_candidate(score, held_codes, p)
    ]


def build_new_buy_queue(scores, held_codes, params=None):
    """Keep >=60 primary candidates ahead of every 50-59 early candidate."""
    p = params or get_default_params()
    primary = []
    for score in sort_candidates(filter_buy_candidates(scores, held_codes, p)):
        item = dict(score)
        item["entry_channel"] = "primary"
        primary.append(item)
    early = []
    for score in sort_candidates(
        filter_early_pre_macd_buy_candidates(scores, held_codes, p)
    ):
        item = dict(score)
        item["entry_channel"] = "early_pre_macd"
        early.append(item)
...
def do_trading(context):
    ...
    late_vetoed = [
        ...
        if is_official_new_buy_candidate(score, buy_exclusions, p)
        and is_late_macd_boll_upper_entry(score)
    ]
    for score in sort_candidates(late_vetoed):
        log.info(
            "[late-macd-boll-veto] %s buy=%.0f close=%.3f upper=%.3f "
            "rsi_age=%s/%s kdj_age=%s/%s macd_age=%s" % (
                score["code"],
                score.get("buy_score", 0),
                score.get("close", np.nan),
                score.get("boll_upper", np.nan),
                score.get("rsi6_cross_rsi12_up_age"),
                score.get("rsi6_cross_rsi24_up_age"),
                score.get("kdj_k_cross_up_age"),
                score.get("kdj_j_cross_up_age"),
                score.get("macd_cross_up_age"),
            ))
    candidates = build_new_buy_queue(all_scores, buy_exclusions, p)
    if not candidates:
        log.info("[%s] no primary or early buy candidates" % STRATEGY_VERSION)
        return
    ...
    for score in candidates:
        ...
        if price is None or price <= 0:
            continue
        if score.get("entry_channel") == "early_pre_macd":
            log.info(
                "[early-pre-macd] %s buy=%.0f rsi_age=%s/%s kdj_age=%s/%s "
                "macd_diff=%.6f prev_diff=%.6f close=%.3f upper=%.3f" % (
                    code,
                    score.get("buy_score", 0),
                    score.get("rsi6_cross_rsi12_up_age"),
                    score.get("rsi6_cross_rsi24_up_age"),
                    score.get("kdj_k_cross_up_age"),
                    score.get("kdj_j_cross_up_age"),
                    score.get("dif", np.nan) - score.get("dea", np.nan),
                    score.get("dif_prev", np.nan) - score.get("dea_prev", np.nan),
                    score.get("close", np.nan),
                    score.get("boll_upper", np.nan),
                ))
        ...
        log.info("[buy] %s channel=%s buy=%.0f rev=%.0f loc=%.0f trend=%.0f vol=%.0f stress=%.2f target=%.0f" % (
            code, score.get("entry_channel", "primary"),
            score["buy_score"], score["reversal_score"], score["location_score"],
