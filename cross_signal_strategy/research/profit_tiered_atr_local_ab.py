# -*- coding: utf-8 -*-
"""Step 1 local A/B: official cross-v0.3.3 versus profit-tiered ATR candidate.
...
profit-tiered variant from the isolated candidate file. Read-only; no formal
...
from cross_signal_strategy.archive.candidates import smart_trade_joinquant_cross_signal_etf_profit_tier_candidate as candidate  # noqa: E402
...
class CandidatePlanner(LocalCrossSignalOrderPlanner):
    """Official planner with the frozen profit-tiered stop-price function."""
    ...
    def _atr_stop_codes(self, broker, current_prices):
        ...
        for code, pos in broker.positions.items():
            ...
            stop_price = candidate.calc_stop_price(
                ...
                pos.avg_cost,
                self.params,
                profit_pct=(float(price) / pos.avg_cost - 1.0),
            )
...
def main():
    ...
    print("本地 A/B: 官方 cross-v0.3.3 vs 利润分段ATR候选 (2019-2021 训练回放)")
    ...
    changed = [
        (base, cand)
        for base, cand in zip(baseline_fills, candidate_fills)
        if base != cand
    ] + baseline_fills[len(candidate_fills):] + candidate_fills[len(baseline_fills):]
    print("-" * 72)
    print("成交路径差异笔数 = %d (买/卖总数 %d vs %d)" % (
        len(changed), len(baseline_fills), len(candidate_fills)))
    for pair in changed[:10]:
