# -*- coding: utf-8 -*-
"""Isolated training candidate: use K/D death cross as ordinary exit.

The official buy path, including MACD scoring, is unchanged. After the frozen
five-trading-day minimum hold, the recent K/D death-cross flag becomes the sole
ordinary signal exit. ATR stops remain unchanged.
"""
...
def candidate_rule_contract() -> dict:
    """Expose the fixed single-variable boundary before running the replay."""
...
@dataclass
class KdjOnlyExitPlanner(LocalCrossSignalOrderPlanner):
    """Official local planner with only the ordinary exit trigger replaced."""
    ...
    def plan_orders(
        ...
        # Preserve the formal same-day-sell exclusion while allowing the next
        # qualified ETF to fill a newly released slot.
...
@dataclass(frozen=True)
class KdjOnlyExitGateDecision:
    passed: bool
    reasons: tuple[str, ...] = ()


@dataclass(frozen=True)
class KdjOnlyExitComparisonReport:
    baseline_report: BaselineReport
    candidate_report: BaselineReport
    baseline: KdjOnlyExitPerformance
    candidate: KdjOnlyExitPerformance
    changed_order_days: int
    changed_days_by_year: Dict[int, int]
    gate: KdjOnlyExitGateDecision


def evaluate_kdj_only_exit_gate(
    baseline: KdjOnlyExitPerformance,
    candidate: KdjOnlyExitPerformance,
    changed_days_by_year: Mapping[int, int],
) -> KdjOnlyExitGateDecision:
    """Require broad training dominance, not one attractive headline metric."""
    ...
    for year in (2019, 2020, 2021):
        baseline_year = baseline.annual_returns.get(year)
        candidate_year = candidate.annual_returns.get(year)
        if baseline_year is None or candidate_year is None:
            reasons.append("%d annual return is missing" % year)
        elif candidate_year < baseline_year:
            reasons.append("%d candidate annual return worsens" % year)
    return KdjOnlyExitGateDecision(not reasons, tuple(reasons))


def run_training_kdj_only_exit_comparison(
    loader=None,
    initial_cash: float = 20000.0,
    warmup_root=APPROVED_WARMUP_ROOT,
) -> KdjOnlyExitComparisonReport:
    ...
    params = strategy.get_default_params()
    if candidate_rule_contract()["min_signal_hold_days"] != 5:
        raise ValueError("Candidate requires the frozen five-trading-day hold")
    ...
    candidate_planner = KdjOnlyExitPlanner(
        _training_adapter(loader, params, warmup_root),
        etf_pool=etf_pool,
        params=dict(params),
        trade_dates=trade_dates,
    ...
    return KdjOnlyExitComparisonReport(
        ...
        gate=evaluate_kdj_only_exit_gate(
            baseline, candidate, changed_days_by_year
        ),
    )
...
def format_kdj_only_exit_comparison(report: KdjOnlyExitComparisonReport) -> str:
    lines = ["KDJ-only-exit comparison (2019-2021; 2018 warm-up only)"]
...
def main() -> None:
    print(format_kdj_only_exit_comparison(run_training_kdj_only_exit_comparison()))
