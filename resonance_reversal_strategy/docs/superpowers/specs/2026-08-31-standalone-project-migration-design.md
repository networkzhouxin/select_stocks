...
本设计将当前已验证提交 `5e81c07` 迁移为独立 Git 项目。迁移只改变项目边界和开发配置，不改变策略、分析器、测试或文档内容。
...
实施开始前必须重新确认来源 worktree 位于提交 `5e81c07` 且工作区干净。目标目录必须不存在；若已经存在，必须停止并报告，不得覆盖或合并。
...
上述文件在目标项目中必须与来源提交逐文件一致。目标集合和来源集合必须双向一致：缺少文件、多出文件或任一 SHA-256 不一致都视为迁移失败。
...
记录来源仓库、worktree、分支、提交、迁移日期、迁移方式、验证命令和验证结果。该文件只提供溯源，不参与策略运行或研究计算。
