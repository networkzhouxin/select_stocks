# -*- coding: utf-8 -*-
"""
智能ETF量化交易策略 V11.0 - PTrade版（回测+实盘兼容）
=============================================
基于V10.0 PTrade版，核心逻辑100%不变，仅扩大标的池。
从聚宽V11.0逐行移植，所有交易逻辑、参数、信号条件100%不变。

改进：从3只扩展到5只宽基ETF
  - 新增510880红利ETF（与成长股低相关，震荡市提供替代机会）
  - 新增512100中证1000ETF（补充小盘敞口，波动率仓位自动控制风险）
  - 不加510050/159901（与510300/159915高度相关，无分散化价值）
  - 更多不相关标的 → 动量排序有更多选择 → 天然分散收益来源

V11.1教训：分阶段止盈（+20%卖半仓）彻底破坏趋势跟踪逻辑
  - 盈亏比从3.7:1暴跌到1.14:1，总收益从45%跌到6%
  - 趋势策略的核心是"让利润奔跑"，不能人为截断
...
标的池（宽基ETF，5只）：
  ...
  - 510880 红利ETF（低波防御）          ← 新增，与成长低相关
  - 512100 中证1000ETF（小盘弹性）      ← 新增，补小盘敞口
"""
...
def initialize(context):
    ...
    # ---- ETF标的池（5只宽基ETF，PTrade使用.SS/.SZ格式）----
    g.etf_pool = [
        '510300.SS',    # 沪深300ETF（大盘均衡）
        '159915.SZ',    # 创业板ETF（成长弹性）
        '510500.SS',    # 中证500ETF（中盘均衡）
        '510880.SS',    # 红利ETF（低波防御）
        '512100.SS',    # 中证1000ETF（小盘弹性）
    ]
