...
|   |-- dependency_lock_v1.py                     # Bootstrap-safe semantic runtime lock
|   |-- digests.py                                 # Bootstrap-safe canonical digest helpers
|   `-- source_execution_v1.py                    # Controlled loaded-source/artifact proof
`-- protocols/
|   |-- __init__.py                               # Post-guard hydrated aggregate exports
|   |-- canonical_json_v1.py                      # Bootstrap canonical protocol leaf
|   |-- float64_v1.py                             # Post-guard NumPy bridge
|   `-- numeric_v1.py                             # Bootstrap numeric protocol leaf
...
- Revision B was approved after Task 2 commit `6a959e84190d41eb8e8d23a90a73bb1455ce4382` and does not recreate the worktree. Before resuming Task 2, commit this revised plan on the design branch, record its raw SHA-256 in the SDD ledger, require implementation-worktree HEAD exactly `6a959e84190d41eb8e8d23a90a73bb1455ce4382`, require the only uncommitted product-tree diff to be the known 417-line addition in `tests/binance_spot/test_source_execution.py`, regenerate `task-2-brief.md`, and stop for review on any other drift.
...
- Modify: `binance_spot_strategy/identity/dependency_contents_v1.py`
- Modify: `binance_spot_strategy/identity/dependency_lock_v1.py`
- Modify: `binance_spot_strategy/identity/digests.py`
- Modify: `binance_spot_strategy/identity/__init__.py`
...
- Produces runtime-only opaque `AttestedExecutionSessionV1`, `AttestedExecutionAdmissionV1`, and `AttestedAdmissionRequirementV1`; none has a public constructor, parser, serializer, equality-by-fields, pickle/reduce hook, or cross-process representation.
...
- Produces production-only C-wrapped `establish_attested_execution(policy: SourceExecutionPolicyV1, content_lock: DependencyContentLockV1) -> AttestedExecutionSessionV1`; it internally performs the real full scan, accepts no caller-supplied verified value, and returns `production_policy_not_frozen` until Task 9 freezes the exact policy-lock bytes. After Task 9 it accepts only that exact loaded policy/content-lock identity, not an arbitrary schema-valid policy.
- Produces production-only C-wrapped `issue_m2_admission(session: AttestedExecutionSessionV1) -> AttestedExecutionAdmissionV1` and `require_attested_m2_admission(value: object) -> AttestedExecutionAdmissionV1`; both accept only production-registry objects.
- Produces production-only C-wrapped `bind_attested_admission_requirement(session: AttestedExecutionSessionV1) -> AttestedAdmissionRequirementV1`; the opaque requirement verifies only admissions from that exact registered production session and grants no issue/register authority.
- Produces module-private C-wrapped `_establish_task2_synthetic_execution(...)`, `_issue_task2_synthetic_admission(...)`, `_require_task2_synthetic_admission(...)`, and `_bind_task2_synthetic_admission_requirement(...)` for the literal `task2-synthetic` profile. Their registry and admission/requirement scope are disjoint from production and none is exported by `identity.__init__`.
- Produces named C-wrapped `require_attested_admission_requirement(value: object) -> AttestedAdmissionRequirementV1`. It authenticates only an already-registered production or synthetic requirement and returns that same opaque requirement; it cannot issue an admission, create/bind a requirement, reveal its scope/session, or accept a structural lookalike. Each requirement is itself a C-wrapped callable with `__wrapped__` removed and validates an admission only against its exact hidden session/scope registry.
...
- M1 synthetic candidate/run objects and supplied digest labels are rejected by both admission-requirement closures.
...
M2 is a controlled-execution attestation and fail-closed import gate for a cooperatively executing process launched by the reviewed isolated launcher. It is not a same-process sandbox. `Denied before resolver delegation, bytes, module creation, or native loading` applies to imports mediated by the persistent guard and to audited Python file APIs after guard installation. Arbitrary already-executing Win32, `ctypes`, C-extension, tracing, GC-referent, monkeypatch, or process-memory capability is outside the M2 boundary. Such capability may read protected paths, alter Python state, close handles, or load code; M2 can invalidate a later admission when observable state drifts but cannot prevent or undo those actions. Stronger containment requires a separately approved restricted-token/AppContainer worker or isolated broker with OS-enforced filesystem access.

Production establish/issue/require authority follows Task 1's ordinary-reflection boundary. The complete lifecycle and weak identity registries live inside one private factory and are exposed only through the specifically named CPython C call wrappers with `__wrapped__` removed; the factory and every raw or generic register/write-capable endpoint are deleted from module globals, while only the named production wrappers and the separately scoped named synthetic wrappers remain. Module attributes, exported-callable globals/defaults/keyword-defaults/closures, wrapper back-references, serialization, copied fields, and `object.__setattr__` cannot mint or authenticate a production admission outside those named wrappers. GC traversal, tracing, native memory, and arbitrary hostile same-process code remain outside the stated boundary. Hidden registry records, not mutable presentation fields, bind PID, exact policy/closure hashes, production-or-synthetic scope, guard identity, retained handles, process image state, and governed module identities. A separate hard-coded synthetic profile accepts only literal `task2-synthetic`; it has a distinct registry and can never satisfy production `require_attested_m2_admission`. There is no caller-supplied `test_only` switch or generic internal establisher.

Dependency admission mode is frozen to complete verified-owner trees: a dependency source, native module, or package-data file is admissible only when its exact file identity, logical path, size, and digest are entries in one of `dependency_owner_tree_sha256s`. Task 1 continues to verify the complete seven-owner content lock, including pandas, but the M2 execution policy admits only the exact ordered runtime, stdlib, NumPy, python-dateutil, six, and tzdata owner trees. The pandas owner tree is deliberately excluded from `dependency_owner_tree_sha256s`; for every guard-mediated import, `pandas`, every `pandas.*` descendant, `pyarrow`, and every `pyarrow.*` descendant must fail before resolver delegation, byte reads, module creation, or controlled native loading. This permits legitimate lazy imports only inside the execution-admitted owner trees while rejecting every resolver result outside them. Business and bootstrap modules remain exact per-module allowlists. Executing pandas/PyArrow is deferred to a separately approved M3 attestation decision and must never be inferred from their installation in the shared interpreter.

- [ ] **Step 1: Write failing policy/schema tests**

Reject missing/unknown fields, invalid versions, bool-as-int, subclasses, duplicate/NFC-colliding modes/modules, noncanonical field-specific ordering, mode not in the internal literal allowlist, bad role, escaping path, invalid callable, omitted bootstrap-closure member, and digest mismatch. Bootstrap modules use the fixed order `binance_spot_strategy.protocols.numeric_v1`, `binance_spot_strategy.protocols.canonical_json_v1`, `binance_spot_strategy.identity.digests`, `binance_spot_strategy.identity.dependency_lock_v1`, `binance_spot_strategy.identity.dependency_contents_v1`, `binance_spot_strategy.identity.source_execution_v1`; business modules use ascending NFC-normalized module-name order; production entrypoints use exact semantic order `verify-m2`, `seal-historical-data`; and owner trees use exact semantic order runtime, stdlib, NumPy, python-dateutil, six, tzdata. Synthetic policy contains exactly one literal `task2-synthetic` entrypoint. The final Task 9 policy uses those same canonical orders and keeps the verified pandas tree absent.

- [ ] **Step 2: Write failing controlled-loader and TOCTOU tests**

Create synthetic packages and attack them with repository shadow modules, altered `sys.path`, replacement of the `sys.meta_path` list, guard removal/reorder/duplication/replacement, replacement of the `sys.modules` dictionary, prefilled or replaced governed modules, custom `MetaPathFinder`, memory/zip/namespace loaders, sourceless `.pyc`, a malicious timestamp-valid cache, symlink/junction escape, source exchange between verify and exec, post-exec disk mutation, forged `module.__file__`, a dependency import outside verified owner trees, and an unlisted extension/DLL load. Add direct, descendant, prefilled-`sys.modules`, and hostile-finder attempts for both `pandas` and `pyarrow`; prove guard-mediated attempts fail before any package byte, package-data byte, extension, or controlled DLL load becomes visible.
...
Add exact six-file bootstrap phase-order, no-aggregate-package-init, no-pre-guard-third-party-import, real-Task-1-verifier-order, ordinary-reflection authority inventory, copied/pickled/cross-process admission and requirement, structural-lookalike requirement, presentation-field mutation, dead/replaced guard, content change before admission, and Windows native swap-and-restore tests. The ordinary-reflection inventory checks module attributes, exported-callable globals/defaults/keyword-defaults/closures, wrapper back-references, writable weak registries, raw register/sign endpoints, and generic establish routes while allowing only the specifically named C wrappers. A synthetic-scope admission or requirement must never satisfy the production require endpoint, and the named requirement-authentication endpoint must grant no issue/register authority.

For native files loaded after launcher start, keep a `CreateFileW` read handle that denies write/delete sharing across hash and delegated load, bind volume/file ID, restrict controlled DLL search roots, and retain all reviewed `.pyd`/owned-DLL handles for the session lifetime. Assert swap/delete/alias/load-from-another-path fails. Controlled extension loading validates the exact pinned image before delegation and compares process-image sets before and after loading. An unexpected non-OS image that remains resident and observable invalidates the session at the next admission issue/require check; a transient out-of-boundary load/unload completed between gates is not detectable and remains outside the cooperative-process boundary. CPython and OS loader images already resident before launcher start remain explicit local TCB, not retrospectively attested memory pages. Do not require pure Python to prevent arbitrary native reads or loads performed by already-executing out-of-boundary code.
...
Start from the 17 uncommitted independent-review tests. Preserve their behavioral coverage, but mechanically rewire fixtures to the hard-coded synthetic C-wrapper lifecycle and rewrite bootstrap coverage to exercise the complete six-binding bootstrap entry. Replace only the obsolete arbitrary-native-read-prevention behavioral assertion with audited Python-path denial plus observable unexpected-process-image invalidation coverage. Record which tests are existing GREEN characterization coverage and the exact RED failures for the absent six-file bootstrap/guard handoff, reachable lifecycle authority, incomplete guard/sys.modules liveness, missing audited denial record, and missing controlled-native image validation before production edits.
...
The launcher contains a standard-library-only strict bootstrap-policy parser, SHA-256/path verifier, minimal phase guard, private package-shell builder, and direct-source loader. Before the first bootstrap module executes, the minimal guard is uniquely installed at `sys.meta_path[0]`; it admits only builtin/frozen/stdlib imports plus the six pre-read reviewed buffers and denies PathFinder delegation to repository or site-packages roots. The launcher reads each of the six exact bootstrap files once, compares its raw bytes to the ordered reviewed binding, and uses that same immutable buffer for SHA-256, `compile`, and `exec`. It creates private `binance_spot_strategy`, `protocols`, and `identity` package shells without executing their aggregate `__init__.py` files, then executes the six buffers in the frozen order with correct package context.

To make the six-file closure real rather than nominal, `dependency_contents_v1.py` and `source_execution_v1.py` import the canonical protocol from `protocols.canonical_json_v1`, `dependency_lock_v1.py` imports numeric primitives from `protocols.numeric_v1`, and `digests.py` imports the canonical protocol from `protocols.canonical_json_v1`; none imports the aggregate `protocols` package surface. A normal import of any bootstrap member, aggregate package initialization, unbound local source read, or NumPy/other third-party import before persistent-guard installation is a hard failure.

The pre-verifier bootstrap TCB is stated exactly: the already-running CPython/OS loader, reviewed launcher bytes, minimal phase guard, private package-shell construction, six ordered reviewed bootstrap bindings, and the exact transitively loaded runtime/stdlib modules and native images required by those sources, including `_decimal`, `ctypes`/`_ctypes`, and importlib machinery when observed. Record that exact set in the bootstrap trace. Task 1 subsequently verifies their installed bytes but does not retroactively attest execution before verification. The protocol proves reproducibility relative to this local TCB; it does not claim TPM, secure boot, code signing, remote attestation, a hostile same-process sandbox, or proof of pre-launch in-memory pages.

Before any non-bootstrap business module or third-party distribution import, the private launcher bootstrap lifecycle used by both the hard-coded Task 2 synthetic path and the later frozen production path calls Task 1's real verifier across the complete content lock, including pandas, and receives its private-issued capability. It pins the exact Revision A execution-owner subset, creates the private immutable source/package-data snapshots and resource reader, installs the persistent import guard as the unique new `sys.meta_path[0]`, and installs the persistent audit/resource guard. It challenges both persistent guards, verifies their exact identities and the complete handoff, and only then removes/deactivates the minimal guard so there is no unguarded or unaudited interval. Only after that atomic handoff may package-shell hydration or third-party module execution begin. The controlled loader uses one execution snapshot per source and retains both persistent guards. Pin every admitted dependency source/native/package-data file with a read handle denying write/delete sharing for the session lifetime; bind file ID/digest and restrict controlled DLL search roots. Retained handles deny write/delete replacement but do not deny arbitrary reads by the same Windows security principal.

After both persistent guards are live, hydrate the existing private package shells by same-buffer execution of the exact business binding `binance_spot_strategy.protocols` from `binance_spot_strategy/protocols/__init__.py`, then `binance_spot_strategy.identity` from `binance_spot_strategy/identity/__init__.py`, then `binance_spot_strategy` from `binance_spot_strategy/__init__.py`. The guard may load their already-allowlisted dependencies, including `float64_v1`, NumPy, `manifests_v1`, and config modules, only from pinned snapshots. Hydration retains each shell's exact object identity, records its loader/spec/origin and pre/post export state, and completes before any admitted consumer observes aggregate exports. A normal import must never treat an unhydrated shell as initialized; replacement, duplicate initialization, or a changed aggregate binding invalidates the session.

The already-live persistent audit hook denies and records Python `open`/`os.open`/pathlib/direct-file-backed-import-resource access under protected roots except the exact private verified resource reader. Source and package-data bytes are read once from verified retained handles into private immutable snapshots; controlled loaders and registered resource readers serve only those snapshots. Native direct reads are outside this guarantee. Builtin/frozen modules require exact reviewed names/loader kinds. Reject pyc, zip/custom/namespace loaders, unsafe controlled DLL search, undeclared business imports, and any unmediated package-data path access observed by the offline trace. Do not add a `six.moves` or generic custom-loader exception in M2: the approved trace must not request it, and any future real requirement for it is another stop-and-revise event.

Task 2 tests the engine only through the hard-coded `task2-synthetic` lifecycle profile. The literal `verify-m2` and `seal-historical-data` modes are reserved and must fail `production_policy_not_frozen` until Task 9 creates the final M2 module lock and enables the separate production lifecycle. No Task 2 or Task 8 helper can mint or authenticate a production admission early, mutable presentation state cannot change registry scope, and command-line input never selects an arbitrary module/callable.
...
At establishment, every guarded import, admission issue, and admission require, validate the exact PID and registered object identity; captured `sys.meta_path` list identity; the unique exact guard at index zero; captured `sys.modules` dictionary identity; absence of pandas/PyArrow roots; exact governed module object/loader/spec/origin identities with no extra governed descendants; retained handle/file-ID/size/digest state; process executable identity; approved non-OS process-image set; hidden policy/scope/closure state; and private audit-guard/resource-reader identities. Before controlled loading, no business or admitted third-party module may be prefilled; afterwards each governed name must resolve to the exact recorded object. Removal, reorder, duplication, replacement, mutation, or unexpected governed descendants invalidates the session.

`issue_m2_admission` proves live session/guard/issuer identity, verifies every retained dependency handle/file ID/digest, reruns complete set/business-source verification, verifies closure, and rejects any unexpected resident process image before returning. The guard repeats source/native checks before lazy load; package-data reads are served only from the verified immutable snapshot/resource reader. Test post-admission package-data mutation through `open`, `os.open`, `Path.read_bytes`, direct file-backed import resources, and alias paths; each audited Python route either returns the originally verified snapshot through the private reader or fails before bytes are visible and records the denial. Preserve native swap/delete and closed-handle/file-ID liveness tests. A controlled unapproved image path must fail before delegation; an image introduced outside the controlled loader and still resident at the next issue/require gate must fail that gate. A transient out-of-boundary load/unload between gates is outside M2. If current packages require unmediated native package-data I/O, DLL search cannot be reduced to exact approved roots, an admitted import introduces an unowned image, or the environment cannot retain the complete handle set, stop and revise the attestation design.
...
Under the controlled launcher, import the exact module names `numpy`, `dateutil`, `six`, `tzdata`, and the existing M1 Binance Spot modules without reading market data. Record the bootstrap phase order, guard handoff, Python audit events, module origins/loaders, controlled extension pre/post image deltas, final process-image classification, and governed `sys.modules` identities while policy identity binds their complete execution-admitted owner trees. Do not import pandas as part of the successful M2 trace. In separate guard-mediated negative probes, require `pandas`, every `pandas.*` descendant, `pyarrow`, and every `pyarrow.*` descendant to fail before `PathFinder`, module execution, protected-root reads, or controlled native-image delegation; assert that none remains in `sys.modules` or the process image set. The Task 1 receipt must still prove the unchanged full content lock, including pandas. The successful trace must observe no `six.moves`; if dynamic code generation, an unowned namespace, an unlisted DLL/plugin, an unexpected non-OS image, an unmediated resource path read, or any required custom loader is observed, stop and revise the attestation design rather than adding a generic exception. Pandas/PyArrow execution attestation is deferred to M3, where it requires a separately reviewed negative-optional-dependency rule or complete owner-tree lock before any training use.

Run the deterministic real-lock trace test exactly:

```powershell
& 'D:\Programs\Python\Python313\python.exe' -B -m unittest tests.binance_spot.test_source_execution.SourceExecutionOfflineTraceTests.test_revision_b_real_lock_trace -v
```

That test launches the fixed `task2-synthetic` profile in a child process with exact `-I -S -B`, accepts no caller module/callable/path, and asserts the canonical trace summary. Append the child command, policy/lock hashes, bootstrap/runtime TCB module-image list, handoff events, governed module identities, final image classification, and pass/fail result to the ignored `task-2-report.md`; no machine path, PID, or timestamp enters stable identity.
...
**Interfaces:**
- Consumes: Task 2 `AttestedExecutionAdmissionV1` plus an exact same-scope `AttestedAdmissionRequirementV1`, a physically training-scoped released root, an opaque approved release, exact `TrainingReleaseRecordV1` closure, `TrainingStageId`, declared indicator/performance windows, and expected manifest hashes.
- Produces: runtime-only opaque `VerifiedTrainingReleaseRootV1`, `ApprovedTrainingReleaseV1`, `ApprovedTrainingReleaseRequirementV1`, and `TrainingReleasedViewV1`; none is publicly constructible, serializable, picklable, exported, or comparable by fields.
- Produces no public or real-release `ApprovedTrainingReleaseV1` factory in M2. The only M2 approval scope is the literal `m2-acceptance-golden`, backed by a disjoint hidden registry and the one exact committed synthetic `TrainingReleaseRecordV1` golden hash; it can never approve an arbitrary or real release.
- Produces module-private named C-wrapped zero-argument `_issue_m2_acceptance_golden_release() -> ApprovedTrainingReleaseV1` and `_bind_m2_acceptance_golden_requirement() -> ApprovedTrainingReleaseRequirementV1`. They accept no caller value, are not exported by `data.__init__` or selectable from CLI input, and can mint/verify only that exact acceptance golden. The requirement is itself a C-wrapped callable with `__wrapped__` removed; its raw issuer, factory, register endpoint, and registry are deleted from module globals.
- Produces module-private C-wrapped `bind_training_released_view_issuer(admission_requirement: AttestedAdmissionRequirementV1, approval_requirement: ApprovedTrainingReleaseRequirementV1) -> Callable[[AttestedExecutionAdmissionV1, VerifiedTrainingReleaseRootV1, ApprovedTrainingReleaseV1], TrainingReleasedViewV1]`. At bind time it first calls Task 2's named C-wrapped `require_attested_admission_requirement`, then authenticates the approval requirement against its own hidden exact-golden registry, and fails before any root/path/content operation. It never accepts a caller callable, boolean, digest, generic verifier, or structural lookalike. The returned bound `issue_training_released_view` is also a C wrapper with `__wrapped__` removed; the raw bound closure, factory, register/write endpoints, and registries are unreachable by ordinary reflection and deleted from module globals. Neither binder nor bound issuer is exported by `data.__init__`.
...
At each call, the bound C-wrapped `issue_training_released_view` first invokes its exact registered admission requirement, then its exact registered `m2-acceptance-golden` approval requirement, before any root/path/content operation. Task 8 binds the synthetic admission requirement; Task 9 binds the production admission requirement, but both use only the acceptance-golden approval scope. This Task 9 object is an acceptance-only composition that exercises the production admission gate; it is not a production release-approval closure. A synthetic admission/requirement never validates a production admission, and the production requirement never validates a synthetic admission. After admission/approval, the bound issuer rechecks the pinned root volume/file ID, reads the single root pointer and release record each exactly once under retained handles, requires the pointer/record hash to equal the approved hash, verifies `training_release_root_identity_sha256` against the current canonical path-free root audit payload, and verifies the record's exact source-contract/raw/stage/catalog hashes. Only then does it parse `TrainingReleaseCatalogV1` and verify every referenced object. The view binds that approved record hash, an internal training-only one-snapshot reader, catalog/manifest refs, and root identity; it exposes no root/path/general content lookup and cannot resolve an object absent from the catalog.
...
Attack both the leaf-module binder and its returned bound issuer through module attributes, callable globals/defaults/keyword-defaults/closures, wrapper back-references, `__wrapped__`, `object.__new__`, copy/deepcopy, pickle/cross-process replay, mutated presentation fields, wrong-session/wrong-scope requirements, forged approval requirements, and direct leaf-module calls. Prove no raw mint/register/factory/registry authority is ordinarily reachable; binder authentication fails before any root spy, and bound-issuer invocation always rejects admission first and approval second before root/path/content/cache spies.
...
Task 8 exercises the lifecycle binder only with the private synthetic session/admission issuer, its same-scope synthetic `AttestedAdmissionRequirementV1`, and the named zero-argument `m2-acceptance-golden` approval/requirement wrappers that recognize the exact committed synthetic `TrainingReleaseRecordV1` golden hash. The binder and requirements are not exported or reachable from CLI input and reject all caller-created callables, strings, records, pass objects, and structural lookalikes. The resulting C-wrapped bound issuer verifies admission and approval before record/catalog/root content, supplies the loader only with the opaque training view, validates declared windows against frozen config without opening bar content, then validates the complete record/catalog/manifest/object snapshot, separates warm-up from performance, and returns exact paired `Bar` buckets.

No production admission issuer, production admission requirement, or production-bound composition exists in Task 8. Task 9 first freezes the complete M2 policy/module lock, then enables production session/admission/requirement issuance and calls only the already named C-wrapped `bind_training_released_view_issuer` with that production requirement plus the private `m2-acceptance-golden` requirement. It never calls or restores the deleted raw lifecycle factory or registry. The result remains an acceptance-only composition and cannot approve a real release.
...
- Produces exact launcher maintenance command `freeze-m2-policy`; it has no caller-selected path/module/callable, exclusively creates only `binance_spot_strategy/config/m2_business_modules.lock.json`, never establishes a session, and fails if the target already exists.
...
Create `m2_business_modules.lock.json` from the exact launcher, the exact ordered six-module bootstrap closure (`numeric_v1`, `canonical_json_v1`, `digests`, `dependency_lock_v1`, `dependency_contents_v1`, `source_execution_v1`), both literal entrypoint bindings, and only the exact execution-admitted runtime, stdlib, NumPy, python-dateutil, six, and tzdata owner trees after all source is present. The exact ascending module-name business inventory is:

```text
binance_spot_strategy
binance_spot_strategy.config
binance_spot_strategy.config.frozen
binance_spot_strategy.data
binance_spot_strategy.data.acquisition_v1
binance_spot_strategy.data.http_records_v1
binance_spot_strategy.data.kline_codec_v1
binance_spot_strategy.data.preregistration_sealer_v1
binance_spot_strategy.data.raw_archive_v1
binance_spot_strategy.data.raw_manifest_v1
binance_spot_strategy.data.stage_access_v1
binance_spot_strategy.data.stage_loader_v1
binance_spot_strategy.data.stage_manifest_v1
binance_spot_strategy.data.transport_v1
binance_spot_strategy.domain
binance_spot_strategy.domain.models
binance_spot_strategy.entrypoints
binance_spot_strategy.entrypoints.seal_historical_data
binance_spot_strategy.identity
binance_spot_strategy.identity.manifests_v1
binance_spot_strategy.protocols
binance_spot_strategy.protocols.float64_v1
```

The launcher hardcodes this tuple, rejects every missing/extra `.py` module outside the six bootstrap members and launcher, hashes each tracked/raw source once, and uses exclusive creation with flush/fsync for the fixed lock path. The fully verified pandas owner tree remains absent from the M2 execution policy, and both pandas and PyArrow roots remain fail-closed. The lock excludes itself and audit-only source commit to avoid recursive tracked identity; the future run identity separately binds the source commit.

Run the one-time deterministic generator and its lock verifier exactly:

```powershell
& 'D:\Programs\Python\Python313\python.exe' -I -S -B binance_spot_strategy/attested_launcher.py freeze-m2-policy
& 'D:\Programs\Python\Python313\python.exe' -B -m unittest tests.binance_spot.test_m2_acceptance.M2PolicyLockTests -v
```

Tests generate only into a temporary repository fixture, prove first create succeeds and rerun/overwrite/missing/extra/reordered/hash-drift cases fail, then verify the checked-in lock parses to the exact tuple above. Record the final lock SHA-256, launcher hash, six bootstrap hashes, business source/artifact hashes, entrypoint order, owner-tree order, and exclusive-create result in the ignored Task 9 report before enabling production. After the fixed lock is exclusively created, do not modify the launcher, any bootstrap or business source, or any policy-bound configuration/input. Production enablement is not a source/config edit: on a later invocation, the same already-hashed code reads that one fixed lock buffer, hashes and parses the same bytes, verifies the exact tuple plus every referenced source/owner identity, and registers only that exact loaded policy/content-lock object. Missing, replaced, or self-inconsistent lock bytes remain fail-closed.

Review the raw launcher/bootstrap/module hashes and the recorded minimal-to-persistent guard handoff, then let the same unchanged fixed-lock loader register the exact policy object and thereby enable only literal `verify-m2` and `seal-historical-data` in the separate production lifecycle, with no mutable test flag, code edit, configuration edit, or generic establishment route. Establish the production `AttestedExecutionSessionV1`, bind its production `AttestedAdmissionRequirementV1`, issue an admission, obtain the zero-argument `_bind_m2_acceptance_golden_requirement()` and `_issue_m2_acceptance_golden_release()` capabilities, and call only the named C-wrapped `bind_training_released_view_issuer`. Run Task 8's actual training-view composition through this acceptance-only bound issuer, which exercises the production admission requirement but has authority only over the committed synthetic golden. Compare the same-process closure to `m2_business_artifact_v1.json`; copied, synthetic-scope, mutated, cross-process, dead-guard, unexpected-resident-image admissions and forged/arbitrary release approvals remain rejected. No real release is loader-approved in M2, and the deleted raw lifecycle factory/registries are never restored or called.
...
The initial plan was committed as the approval artifact before the implementation worktree was created. User-approved revisions are committed separately on the design branch and recorded by commit plus raw SHA-256 in the SDD ledger; Task 9 does not stage any design-plan file from the implementation worktree.
