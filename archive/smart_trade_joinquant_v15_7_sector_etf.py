# -*- coding: utf-8 -*-
"""
智能ETF量化交易策略 V15.7-行业版 - 行业ETF动量轮动
=============================================
基于V15.7动量轮动框架，ETF池替换为行业ETF。
策略逻辑、参数、止损体系与V15.7 100%一致，仅ETF池不同。

特点：
  - 比V15.7波动更大、弹性更高（行业ETF波动率是宽基的2-3倍）
  - 预期收益更高，但回撤也更大
  - 适合风险承受力强、追求高收益的投资者

ETF池（6行业+2跨市场+2跨资产 = 10只）：
  A股行业6只：
  - 512660 军工ETF（国防政策，独立周期）
  - 512010 医药ETF（医疗健康，防御+成长）
  - 512400 有色金属ETF（大宗商品周期）
  - 159928 消费ETF（内需消费）
  - 512880 证券ETF（市场情绪，金融周期）
  - 159995 芯片ETF（半导体，科技周期）[2019年上市]
  跨市场2只：
  - 513100 纳指ETF（美国科技，A股弱势时接力）
  - 513500 标普500ETF（美国全行业，系统性分散）
  跨资产2只：
  - 518880 黄金ETF（避险，与行业低相关）
  - 511010 国债ETF（债券兜底）
"""
...
def initialize(context):
    ...
    g.etf_pool = [
        # ---- A股行业 6只 ----
        '512660.XSHG',   # 军工ETF（国防政策）
        '512010.XSHG',   # 医药ETF（医疗健康）
        '512400.XSHG',   # 有色金属ETF（商品周期）
        '159928.XSHE',   # 消费ETF（内需消费）
        '512880.XSHG',   # 证券ETF（金融周期）
        '159995.XSHE',   # 芯片ETF（半导体）[2019上市]
        # ---- 跨市场 2只 ----
        '513100.XSHG',   # 纳指ETF（美国科技）
        '513500.XSHG',   # 标普500ETF（美国全行业）
        # ---- 跨资产 2只 ----
