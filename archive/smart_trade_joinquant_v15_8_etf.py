# -*- coding: utf-8 -*-
"""
智能ETF量化交易策略 V15.8 - 止损防鞭打+止损后国债占位
=============================================
基于V15.7（国债填空优化），新增两项优化：
  4. 止损冷却期：被ATR止损的ETF，2个轮动周期（6个交易日）内禁止买回
     防止"止损后立即买回"的鞭打损耗（V10有5天冷却，V15缺失）
  5. 止损后国债占位：止损释放的资金立即买入国债，避免等到下个轮动日才部署
     下次轮动日国债会被正常换仓逻辑替换

已有特性（V15.7）：
  1. 买入价修正：用T日09:35实时价计算股数和止损基准
  2. 国债兜底：所有权益类ETF动量为负时，自动买入国债ETF
  3. 国债填空：候选不足max_hold时，国债自动补位
  4. 排名公式：sort_score = risk_adj_momentum × 0.7 + trend_strength × 0.3
...
def initialize(context):
    ...
    g.params = {
        ...
        'cooldown_cycles': 2,        # V15.8：止损后冷却轮动周期数（2×3=6个交易日）
    }
    ...
    g.highest_since_buy = {}
    g.entry_atr = {}
    g.stop_cooldown = {}  # V15.8：{code: 剩余冷却天数}
    ...
    for code in list(context.portfolio.positions.keys()):
        ...
        if code in g.highest_since_buy and code in g.entry_atr:
            ...
            if cur_price <= stop_price:
                ...
                # V15.8：记录冷却期
                cooldown_days = g.params['cooldown_cycles'] * g.params['rebalance_interval']
                g.stop_cooldown[code] = cooldown_days
                log.info('[冷却期] %s 止损后冷却%d个交易日' % (code, cooldown_days))
                stopped.append(code)
...
def do_trading(context):
    ...
    # ======== V15.8：更新冷却计数（每个交易日递减）========
    for code in list(g.stop_cooldown.keys()):
        g.stop_cooldown[code] -= 1
        if g.stop_cooldown[code] <= 0:
            log.info('[冷却解除] %s' % code)
            del g.stop_cooldown[code]
    ...
    if stopped_codes:
        ...
        already_hold_bond = bond_pos is not None and bond_pos.total_amount > 0
        if not already_hold_bond and not current_data[bond].paused:
            available = context.portfolio.available_cash
            price = current_data[bond].last_price
            shares = int(available * 0.90 / price / 100) * 100
            if shares >= 100:
                log.info('[止损后国债占位] 买入%s %d股 @%.3f' % (bond, shares, price))
                order(bond, shares)
    ...
    # V15.7：候选不足max_hold时，用国债ETF填满剩余仓位
    bond = g.bond_etf
...
def after_close(context):
    ...
    for code, pos in hold.items():
        ...
        cooldown_info = ' [冷却%d天]' % g.stop_cooldown[code] if code in g.stop_cooldown else ''
        log.info('  %s 成本:%.3f 现价:%.3f 高:%.3f 盈亏:%.1f%%%s' % (
            code, pos.avg_cost, pos.price, highest, profit_pct, cooldown_info))
    log.info('=' * 60)
