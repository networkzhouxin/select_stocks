# -*- coding: utf-8 -*-
"""
智能ETF量化交易策略 V15.9-Hybrid - ROC+LR混合评分实验版
=============================================
基于V15.9，将ROC和线性回归两种动量评分取交集：
  - 过滤层：ROC20>0, ROC60>0, 价格>MA20（V15.9原版，安全网）
  - 过滤层：LR×R² > 0（趋势纯度过滤，排除R²低的震荡标的）
  - 排名层：score = ROC风险调整动量 × 0.5 + LR×R² × 0.5（各占一半权重）
  同时满足ROC和LR两套标准才能入选 = 交集
  每日轮动（无路径偏移），ROC+LR双重过滤抑制噪音换仓
其余框架不变：12只ETF、max_hold=3、ATR止损、国债填空。
...
def initialize(context):
    ...
    # ---- 策略参数 ----
    g.params = {
        'rebalance_interval': 1,     # 每日轮动 + ROC×LR双重过滤抑制噪音
...
def calc_momentum(code, end_date):
    """
    计算单只ETF的动量得分（ROC+LR+ER三维混合版）。
    过滤层：ROC20>0, ROC60>0, 价格>MA20, LR×R²>0（交集）
    排名层：ROC(归一化) × 1/3 + LR×R²(归一化) × 1/3 + ER(归一化) × 1/3
    返回None表示不满足买入条件。
    """
    ...
    roc_score = risk_adj_mom * (1 - tw) + trend_strength * tw
    ...
    lr_score = annualized_return * r2

    # ---- 过滤层2：LR评分也必须为正（交集过滤）----
    if lr_score <= 0:
        return None

    # ---- Efficiency Ratio（效率比，Kaufman）----
    direction = abs(C.iloc[-1] - C.iloc[-mom_period])
    volatility_sum = C.diff().abs().iloc[-mom_period:].sum()
    er_score = direction / volatility_sum if volatility_sum > 0 else 0
...
def do_trading(context):
    ...
    # 分位数归一化：确保ROC、LR、ER三个评分维度等权贡献排名
    if len(candidates) >= 2:
        n = len(candidates) - 1
        ...
        for rank, idx in enumerate(sorted(range(len(candidates)), key=lambda i: candidates[i]['er_score'])):
            candidates[idx]['_er_rank'] = rank / n
        for c in candidates:
            c['momentum'] = c['_roc_rank'] / 3 + c['_lr_rank'] / 3 + c['_er_rank'] / 3
