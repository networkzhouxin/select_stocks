# -*- coding: utf-8 -*-
"""
智能ETF量化交易策略 V12.0 - 信号优化版
=============================================
基于V11.0回测数据诊断，优化买入信号质量：

V12.0信号优化（基于53笔实盘回测数据分析）：
  1. BU1增加MA20斜率过滤：原BU1单独触发胜率仅12.5%（8笔7亏），
     根因是MA20走平/下行时的假突破。增加MA20>MA20[5日前]条件。
  2. BU4替换：原底背离信号4年0次触发（死代码）。
     替换为"趋势回踩"信号：上升趋势中价格回踩MA20后企稳反弹。
     与BU2（超卖反转）互补：BU2抓V型急弹，新BU4抓缓慢回调延续。
  3. 买入增加动量过滤：ROC>0。原策略在负动量时仍买入，
     8笔负动量交易6亏2微赚，与趋势跟踪哲学矛盾。
  4. 修复cleanup_position_state未定义的BUG。
...
def calc_indicators(code, end_date, count=120):
    ...
    # BU1: 放量突破MA20（V12.0增加MA20斜率过滤，排除假突破）
    # 原版8笔单独触发仅1赚，根因是MA20走平/下行时的假突破
    MA20_rising = MA20 > MA20.shift(5)
    BU1 = (C > MA20) & (C.shift(1) <= MA20.shift(1)) & (VR > 1.2) & MA20_rising

    # BU2: 极度超卖反转（不变，策略核心引擎，贡献全部大赢家）
    BU2 = ((J0 < 10) | (RSI6 < 20)) & 阳线 & (实体 > 0)

    # BU3: MACD零下金叉（不变）
    BU3 = (DIF0 > DEA0) & (DIF0.shift(1) <= DEA0.shift(1)) & (DIF0 < 0)

    # BU4: 趋势回踩（V12.0替换原底背离，原版4年0次触发）
    # 上升趋势中价格回踩MA20后企稳反弹，与BU2互补
    BU4 = (MA20_rising                            # MA20向上（确认上升趋势）
           & (L.shift(1) <= MA20.shift(1))        # 昨日最低触碰或跌破MA20
           & (C > MA20)                           # 今日收盘站回MA20上方
           & 阳线                                 # 阳线确认反弹
           & (C > C.shift(1))                     # 收盘价高于昨日（企稳）
          )
...
def market_open(context):
    ...
    for code in g.etf_pool:
        ...
        # V12.0: 动量过滤，ROC<=0时不买入（负动量入场胜率仅25%）
        if sig['ROC'] <= 0:
            continue
