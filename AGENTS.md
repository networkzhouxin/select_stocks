...
- **发现问题必须上报，不得自作主张接受**：任何已发现的问题、缺陷、边界情况或"看起来无害"的瑕疵（哪怕自认为影响小、不用改），都必须明确向用户提出，说清问题本质、影响范围和可选方案，由用户决策。不得自行认定"接受现状""无需处理""就这么定了"——决策权在用户，不在助手。
- **任何模糊必须向用户确认**：有任何模糊、不确定、自己搞不清楚的东西（数据来源、回测口径、策略规则、用户意图、事实依据、异常现象等），必须停下来向用户确认，不得猜测、不得默认一个答案、不得自作主张补全。凡是自己拿不准的，一律以用户确认为准。
...
- **Cross-signal warm-up data**: `G:\financial\history_data\cross_signal_warmup_2018` may be used only as a read-only daily-bar lookback buffer for 2019-2021 training signals. It must never be used for performance statistics, parameter tuning, validation, or execution prices.
- **Cross-signal QDII underlying-index data**: The single pre-registered underlying-direction observation may read only `G:\financial\history_data\cross_signal_underlying_train_2018_2021`. Treat that root as immutable: never modify, overwrite, clean, delete, or generate derived files inside it. Accept only 2018 warm-up and 2019-2021 source sessions with final official values and timezone-aware historical `available_at`; China T 09:35 may use only rows whose `available_at` and `session_date` are not later than the decision point. Never substitute ETF prices, assumed close times, validation-period index data, or today's download timestamp for missing point-in-time evidence. Until the root exists and passes the contract, the observation remains blocked and cannot create a strategy candidate.
- **Cross-signal local replay alignment**: JoinQuant remains the authority for strategy performance. Local replay is for fast signal-path debugging, data-quality checks, and training-window structure experiments. For any new time window, first align the order path against JoinQuant filled trades (date, side, code, event count) before interpreting local returns. Do not require local returns to match JoinQuant penny-for-penny because market-order matching prices cannot be fully reproduced from 09:35 minute bars alone. If path alignment fails, investigate local data defects, adjustment factors, new-listing history, halted/zero-volume rows, and boundary precision before changing strategy rules. Confirmed local data defects must be handled through external read-time correction layers; never edit, overwrite, or delete the source market-data folders.
