...
def test_joinquant_exposes_stable_release_fingerprint():
    ...
    assert strategy.DEPLOYMENT_BUILD_ID == "20260727.5"
...
def test_failed_buy_order_consumes_intended_slot_and_does_not_backfill(
        monkeypatch):
    ...
    assert [code for code, _target in order_calls] == [failed_code]
    assert not strategy.has_position(Context(), backup_code)
...
def test_only_confirmed_pause_releases_top_candidate_slot(
    ...
    monkeypatch.setattr(
        ...
        types.SimpleNamespace(
            params=params,
            etf_pool=[paused_code, backup_code],
            highest_since_buy={},
            entry_atr={},
            buy_date={},
            last_scores={},
