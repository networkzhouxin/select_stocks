# -*- coding: utf-8 -*-
"""Tests for the isolated KDJ 50%/25% threshold-weight candidate."""
...
def _module():
    try:
        from cross_signal_strategy.research import (
            kdj_tiered_half_threshold_candidate,
        )
    except ImportError as exc:  # pragma: no cover - exercised in TDD red phase
        pytest.fail("half-threshold KDJ candidate is not implemented: %s" % exc)
    return kdj_tiered_half_threshold_candidate
...
def _adapter(scores_by_date, dates=None):
    module = _module()
    return module.KdjTieredHalfThresholdScoreAdapter(
...
def test_threshold_relative_tiers_adjust_the_unified_scores(
    k_value,
    buy_bonus,
    sell_bonus,
):
    ...
    assert candidate["sell_score"] == 15.0 + sell_bonus
...
def test_training_runner_rejects_unapproved_data_roots():
    ...
    with pytest.raises(ValueError, match="approved training data root"):
        module.run_kdj_tiered_half_threshold_training_ab(
